...
def check_content_moderation(prompt: str, client: Optional[openai.OpenAI]) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if result.flagged:
            ...
            logger.warning(f"Input flagged by moderation for: {', '.join(flagged_categories)}") # CORRECTED LINE
            return {
                "flagged": True, 
...
`DatabaseManager` class: This class is responsible for managing the database connection.
   - It attempts to connect to `SQLite Cloud` if `SQLITECLOUD_AVAILABLE` is True and `connection_string` is provided in the secrets.
   - If `SQLite Cloud` fails or is not configured, it falls back to a local `SQLite` file (`fifi_sessions_v2.db`).
   - If both fail, it resorts to an in-memory `SQLite` database (which is non-persistent).

3. **Initialize the database schema.**

`_init_complete_database()`: This method, called during `DatabaseManager` initialization, ensures that the database schema is correctly set up.
   - It checks if the `sessions` table exists and creates it if it doesn't, including all necessary columns.
   - It also creates essential indexes for common lookup fields (`session_id`, `fingerprint_id`, `email`) to optimize database queries.

4. **Add error handling for SQLite Cloud operations.**

`@handle_api_errors("Database", "Operation")`: This decorator is applied to various database operations within `DatabaseManager` (`save_session`, `load_session`, `find_sessions_by_fingerprint`, `find_sessions_by_email`). It provides a centralized way to catch exceptions during database interactions, log them with relevant context, display user-friendly messages, and potentially allow for fallback mechanisms. This significantly improves the robustness of database interactions.

5. **Fix the session loading logic for `UserSession` objects.**

   - In `DatabaseManager.load_session()`, the `session_id` is now explicitly ensured to be part of the `session_params` dictionary before unpacking it into the `UserSession` constructor. This prevents a `TypeError` if `session_id` was somehow missing from the database row, ensuring the `UserSession` object is always correctly instantiated.
   - In `DatabaseManager.find_sessions_by_fingerprint()` and `DatabaseManager.find_sessions_by_email()`, the same explicit handling for `session_id` is added to ensure that every `UserSession` object created from query results is valid.

6. **Implement Universal Fingerprinting.**

`FingerprintingManager`: This class manages a 3-layer browser fingerprinting mechanism (Canvas, WebGL, AudioContext) to uniquely identify user devices.
   - `generate_fingerprint_component()` creates JavaScript code injected into the frontend to collect these fingerprints.
   - `extract_fingerprint_from_result()` processes the data returned from the JavaScript component, determines `visitor_type` (new vs. returning), and sets `privacy_level`.
   - `_generate_fallback_fingerprint()` provides a unique fallback ID if client-side fingerprinting fails.

7. **Implement a 3-Tier User System with Question Limits.**

   `UserType` Enum: Defines three user tiers: `GUEST`, `EMAIL_VERIFIED_GUEST`, and `REGISTERED_USER`.
   `QuestionLimitManager`: Manages question allowances and daily limits for each user type.
   - `is_within_limits()` checks if a user is allowed to ask a question, applying rolling 24-hour windows.
   - `record_question()` increments question counts.
   - Includes logic for escalating bans (`BanStatus` Enum) and evasion detection.

8. **Implement Email Verification.**

   `EmailVerificationManager`: Manages the process of sending and verifying OTPs via Supabase Auth.
   - `send_verification_code()` integrates with Supabase's `sign_in_with_otp` method.
   - `verify_code()` verifies the OTP provided by the user.

9. **Implement a 15-minute Session Timeout with CRM Saves.**

   `render_activity_timer_component_15min()`: A JavaScript component that tracks user inactivity and triggers a Python event after 15 minutes.
   `handle_timer_event()`: Processes this event, initiating an automatic chat transcript save to Zoho CRM for `REGISTERED_USER` sessions.

10. **Implement Robust Browser Close Detection.**

    `render_browser_close_detection_enhanced()`: A JavaScript component that uses `navigator.sendBeacon` (with `XMLHttpRequest` fallback) to send an "emergency save" signal to the backend when a browser tab is closed or navigated away.
    `handle_emergency_save_requests_from_query()`: A backend function that intercepts these signals via URL query parameters and triggers a CRM save. This ensures that user conversations are preserved even if the browser closes unexpectedly.

11. **Enhance Zoho CRM Integration for Chat Transcripts.**

    `ZohoCRMManager`: Manages interactions with Zoho CRM.
    - `_get_access_token()` securely retrieves/refreshes Zoho OAuth tokens.
    - `_find_contact_by_email()` and `_create_contact()` manage contact records.
    - `_upload_attachment()` uploads PDF chat transcripts to contacts.
    - `_add_note()` adds summary notes to contacts.
    - `save_chat_transcript_sync()` orchestrates the entire process (find/create contact, generate PDF, upload, add note) with retry logic.
    `PDFExporter`: Generates PDF documents from chat message history.

12. **Centralize Session Management.**

    `SessionManager`: This class orchestrates all other managers (`DatabaseManager`, `ZohoCRMManager`, `EnhancedAI`, `FingerprintingManager`, `EmailVerificationManager`, `QuestionLimitManager`).
    - `get_session()`: The primary entry point for retrieving the current `UserSession`, handling loading, creation, activity updates, and ban checks.
    - `_update_activity()`: Updates session timestamps and saves to DB.
    - `authenticate_with_wordpress()`: Handles user login via WordPress JWT.
    - `get_ai_response()`: Integrates rate limiting, content moderation, evasion detection, and AI response generation.
    - `clear_chat_history()`, `end_session()`, `manual_save_to_crm()`: Provide user actions.

13. **Implement Robust Error Handling and Logging.**

    `EnhancedErrorHandler`: Provides a centralized mechanism for handling API errors, classifying them by severity, logging technical details, and displaying user-friendly messages.
    `@handle_api_errors`: A decorator for wrapping API calls to ensure consistent error handling.
    Extensive `logger.info`, `logger.warning`, `logger.error`, and `logger.debug` statements are sprinkled throughout the code for detailed operational visibility.

14. **Improve UI/UX Elements.**

    Streamlit's `st.set_page_config`, `st.columns`, `st.tabs`, `st.form`, `st.text_input`, `st.button`, `st.spinner`, `st.success`, `st.error`, `st.info`, `st.warning`, `st.markdown`, `st.chat_message`, `st.chat_input`, `st.progress`, `st.download_button`, `st.rerun`, and `st.stop` are used to create an interactive and responsive web application.

15. **Address Streamlit-Specific Compatibility Issues.**

    `global_message_channel_error_handler()`: Injects JavaScript to gracefully handle common "message channel closed" errors that can occur with Streamlit's iframe architecture.
    `st_javascript`: Used for executing custom JavaScript components (fingerprinting, activity timer, client info detection).
    Conditional logic (`hasattr(ctx, 'request_context')`, etc.) is used in `_capture_client_info` to adapt to different Streamlit internal API structures across versions.

In summary, the application is designed to provide a resilient, feature-rich AI assistant experience, with a strong focus on user session management, data persistence, and error handling, while navigating the complexities of Streamlit's execution model and external API integrations.

Could you please provide the content of the "error log" that indicates a `NameError` for `fingerprinting_manager`? The code I provided in the previous turn *already includes* the instantiation of `fingerprinting_manager` within `ensure_initialization()`.

I need to confirm if there's a new version of the code you're using or if the previous fix wasn't correctly applied.

Please provide the full, current traceback of the `NameError`.You are absolutely correct! My apologies for that oversight. A `NameError` for `fingerprinting_manager` in `ensure_initialization` can only mean that my previous update to instantiate it there didn't stick, or there's some confusion about the code version.

I understand the problem precisely: `fingerprinting_manager = FingerprintingManager()` needs to be present and executed within the `ensure_initialization()` function before it's passed to the `SessionManager` constructor.

I will ensure this line is correctly placed in the `ensure_initialization` function. I will also re-verify all other recent fixes are still present.

Here is the **definitively fixed and fully integrated code**, with the `fingerprinting_manager` instantiation confirmed, and all previous fixes (enum comparisons, OpenAI model, database, CRM, client info capture, etc.) preserved.
    ...
    def save_session(self, session: UserSession):
        with self.lock:
            ...
            try:
                # Get the actual column names from the database table at runtime
                cursor = self.conn.execute("PRAGMA table_info(sessions)")
                db_column_names = [row for row in cursor.fetchall()] # row is the column name
    ...
    def load_session(self, session_id: str) -> Optional[UserSession]:
        with self.lock:
            ...
            try:
                ...
                row_dict = dict(row) if hasattr(row, 'keys') else dict(zip([d for d in cursor.description], row))
...
class EnhancedAI:
    ...
    def get_response(self, prompt: str, chat_history: List[Dict] = None) -> Dict[str, Any]:
        ...
        #             messages=messages
        #         )
        #         return {"content": response.choices.message.content, "source": "OpenAI", "success": True}
        #     except Exception as e:
        #         logger.error(f"OpenAI API call failed: {e}")
        #         # Fallback to generic response
        #         return {"content": "Sorry, my AI services are currently unavailable.", "success": False, "source": "AI Error"}
...
class SessionManager:
        ...
        try:
            ...
            if headers:
                ...
                for header_name in ip_headers_priority:
                    ...
                    if ip_val:
                        session.ip_address = ip_val.split(',').strip()
    ...
    def _mask_email(self, email: str) -> str:
        ...
        if len(local_part) <= 2:
            masked_local = local_part + '*' * (len(local_part) - 1)
        else:
            masked_local = local_part + '*' * (len(local_part) - 2) + local_part[-1]
        
        domain_segments = domain_part.split('.')
        if len(domain_segments) > 1:
            masked_domain_first_part = '*' * len(domain_segments) 
