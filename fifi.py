...
from production_config import (
    ...
    FASTAPI_FINGERPRINT_URL # NEW: Import FastAPI fingerprint URL
)
...
class WooCommerceManager:
    ...
    @handle_api_errors("12Taste Order Status", "Get Order", show_to_user=True) # Changed component name
    async def get_order(self, order_id: str, customer_id: Optional[int] = None) -> Optional[Dict[str, Any]]: # Added customer_id
        ...
        try:
            ...
            params = {}
            if customer_id:
                params['customer'] = customer_id # Filter orders by customer ID

            response = await asyncio.to_thread( # Use asyncio.to_thread for sync requests
                requests.get,
                f"{self.base_url}/orders/{clean_order_id}",
                auth=self.auth,
                params=params, # Pass params including customer_id
                timeout=10,
                headers={
            ...
            # Additional check: If customer_id was provided, verify the order belongs to them
            ...
            logger.error(f"12Taste Order Status API error: {e}") # Changed component name
            return {"error": "api_error", "message": str(e)}

    @handle_api_errors("12Taste Order Status", "Get Customer by Email", show_to_user=False) # New method, changed component name
    async def get_customer_by_email(self, email: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if customers:
                ...
                return customers[0] # Return the first customer found
            logger.info(f"No customer found for email {email}")
            return None
        except requests.exceptions.RequestException as e:
            logger.error(f"12Taste Order Status API error retrieving customer for {email}: {e}") # Changed component name
            return None
...
"""
    ...
    @handle_api_errors("12Taste Order Status", "Test Connection", show_to_user=False) # Changed component name
    def test_connection(self) -> Dict[str, Any]:
        """Test WooCommerce API connection and return status information."""
        if not self.config.WOOCOMMERCE_ENABLED:
            return {
                "status": "disabled",
                "message": "12Taste Order Status integration is not enabled" # Changed message
            }
        ...
        try:
            ...
            if response.status_code == 200:
                return {
                    "status": "connected",
                    "message": "12Taste Order Status API is working correctly", # Changed message
                    "api_url": self.base_url
                }
            elif response.status_code == 401:
                return {
                    "status": "authentication_failed",
                    "message": "12Taste Order Status authentication failed. Check API credentials." # Changed message
                }
            else:
                return {
                    "status": "error",
                    "message": f"12Taste Order Status API returned status code: {response.status_code}" # Changed message
                }
                
        except requests.exceptions.Timeout:
            return {
                "status": "timeout",
                "message": "12Taste Order Status API request timed out" # Changed message
            }
        except requests.exceptions.ConnectionError:
            return {
                "status": "connection_error",
                "message": "Could not connect to 12Taste Order Status API" # Changed message
            }
        except Exception as e:
            return {
                "status": "error",
                "message": f"12Taste Order Status API error: {str(e)}" # Changed message
            }
                    ...
                    if session.user_type != UserType.REGISTERED_USER and \
                       ...
                       not st.session_state.get(fingerprint_checked_key, False):
                        ...
                        logger.info(f"Fingerprint inheritance check and save completed for {session.session_id[:8]}")
                         ...
                         (session.fingerprint_id is None or session.fingerprint_id.startswith(("temp_", "fallback_", "temp_py_"))):
...
### **2. Updated `fingerprint_component.html`**

This file will now use `fetch` to send data to your FastAPI backend and a light `Image` beacon to signal Streamlit.

```html
<!-- fingerprint_component.html -->
<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8" />
...
(function () {
  ...
  // Use sessionStorage to prevent duplicate processing on rapid reruns within same browser session
  ...
  // Function to signal Streamlit that client-side fingerprinting is done.
  // This triggers Streamlit to check its DB for the FP data that FastAPI has saved.
  function sendPingToStreamlitReadyState(status = 'complete', fpId = '') {
      // Ensure we don't spam pings
  ...
  // Primary function to send fingerprint to FastAPI backend
  async function sendFingerprint(payload) {
    if (fingerprintSent) return;
    fingerprintSent = true;
    sessionStorage.setItem(PROCESSED_KEY, 'true'); // Mark as sent in session storage
    ...
    try {
        const response = await fetch(fastapiFingerprintUrl, {
            ...
            keepalive: true // Important for reliable sending during page unload/close
        });
        ...
        // Always signal Streamlit completion, regardless of FastAPI success/failure,
        // so Streamlit knows it can stop waiting and check its DB.
  ...
  function tryDomScrape() {
    ...
    if (stable && stable !== 'Computing...') {
      ...
      sendFingerprint({ // Call the new FastAPI sender
  ...
  }, fingerprintTimeoutSeconds * 1000); // Absolute timeout using injected constant
})();
</script>
</body>
</html>
...
### **3. Updated `main.py` (FastAPI Backend)**

No changes are strictly required here as your `/fingerprint` endpoint is already well-defined to receive the payload. However, for completeness, I'll include the relevant parts.
...
# Ensure 'allow_origins' includes your Streamlit app's domain, e.g., "https://fifi-eu-121263692901.europe-west1.run.app"
# And the parent domain, e.g., "https://www.12taste.com"
app.add_middleware(
    CORSMiddleware,
...
class UserSession:
    ...
    timeout_detected_at: Optional[datetime] = None
    timeout_reason: Optional[str] = None
    # Add new fields if they exist in UserSession in main_fixed.py
    current_tier_cycle_id: Optional[str] = None
    tier1_completed_in_cycle: bool = False
    tier_cycle_started_at: Optional[datetime] = None
    login_method: Optional[str] = None
...
def is_session_ending_reason(reason: str) -> bool:
    ...
    session_ending_keywords = ['beforeunload', 'unload', 'close', 'refresh', 'timeout', 'browser_close', 'tab_close', 'visibility_hidden_background']
...
class ResilientDatabaseManager:
    ...
    async def _ensure_connection(self):
        ...
        if not self.conn:
            self.db_type = "memory"
            self._initialized_schema = True # Assume schema exists for memory
    ...
    async def save_session(self, session: UserSession):
        ...
        session_dict = {
            ...
            "current_tier_cycle_id": session.current_tier_cycle_id,
            "tier1_completed_in_cycle": int(session.tier1_completed_in_cycle),
            "tier_cycle_started_at": session.tier_cycle_started_at.isoformat() if session.tier_cycle_started_at else None,
            "login_method": session.login_method,
            "is_degraded_login": int(session.is_degraded_login),
            "degraded_login_timestamp": session.degraded_login_timestamp.isoformat() if session.degraded_login_timestamp else None
        }
...
async def startup_event():
    ...
    await db_manager.test_connection() # Test connection on startup
...
async def receive_fingerprint(payload: FingerprintPayload):
    ...
    try:
        # Load the session (could be inactive if the user closed the tab, then reopened, and this is an old session)
        session = await db_manager.load_session(payload.session_id)
        if not session:
            # If active session not found, try to load any session to update its fingerprint
            # This is important if a user closes a session, but the FP beacon still fires.
            # We want to associate the FP with the correct session, even if it's currently inactive.
        ...
        if session.user_type == UserType.REGISTERED_USER and session.fingerprint_id != "not_collected_registered_user":
            logger.warning(f"Attempted to process fingerprint from FastAPI for REGISTERED_USER {session.session_id[:8]}. Ignoring as fingerprint is not collected for this user type.")
            return {
                "status": "ignored",
                "message": "Fingerprint not applicable for registered users."
            }
        ...
        session.last_activity = datetime.now() # Mark activity on FP receipt
...
### **4. Updated `main_fixed.py` (Streamlit Frontend)**
...
# NEW: Handle the specific 'fingerprint_status_update' event from the iframe
...
def main_fixed():
    ...
    if 'initialized' not in st.session_state:
        defaults = {
            ...
            "fallback_session_id": '',
            "fingerprint_client_side_completed": False, # NEW: Client-side FP status
            "expecting_order_id": False # NEW: For WooCommerce order flow
        }
    ...
    # handle_fingerprint_requests_from_query() # REMOVED: No longer handles actual FP data
    if handle_fingerprint_status_update_from_query(): # NEW: Handle iframe signaling completion
        return # Stop execution if a fingerprint status update was handled and reran
        ...
        try:
            ...
            if session:
                if session.user_type == UserType.REGISTERED_USER:
                    ...
                    # After initial load/auth, check DB for stable FP
                    latest_session_from_db = session_manager.db.load_session(session.session_id)
    ...
    if current_page != "chat":
        ...
        # Conditionally render fingerprinting component ONLY IF client-side hasn't completed yet
        # and it's a non-registered user
        if session.user_type != UserType.REGISTERED_USER and not st.session_state.get('fingerprint_client_side_completed', False):
        ...
        if session.user_type != UserType.REGISTERED_USER:
            # Check the DB for the latest fingerprint after the client-side component has signaled completion.
            # This is crucial because FastAPI updates the DB, not Streamlit's session_state directly.
            if st.session_state.get('fingerprint_client_side_completed', False):
                latest_session_from_db = session_manager.db.load_session(session.session_id)
                if latest_session_from_db:
                    session.fingerprint_id = latest_session_from_db.fingerprint_id
                    session.fingerprint_method = latest_session_from_db.fingerprint_method
                    session.browser_privacy_level = latest_session_from_db.browser_privacy_level
                    # Clear the client-side completed flag once we've read from DB
                    st.session_state.fingerprint_client_side_completed = False
            ...
            if fingerprint_is_stable:
                ...
                if time.time() - st.session_state.fingerprint_wait_start > FINGERPRINT_TIMEOUT_SECONDS:
                    ...
                    st.session_state.is_chat_ready = False
        else: # Registered users are always ready (fingerprint handled as "not collected" or inherited by email)
            st.session_state.is_chat_ready = True
            if 'fingerprint_wait_start' in st.session_state:
                del st.session_state['fingerprint_wait_start']
        ...
        if not st.session_state.get('is_chat_ready', False):
            # NEW: Show a spinner or message directly in the chat area
            with st.chat_message("assistant", avatar=FIFI_AVATAR_B64):
