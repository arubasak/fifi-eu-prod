...
from typing import List, Dict, Optional, Any, Callable, Generator # Added Generator for streaming return type
...
class SessionManager:
    ...
    def handle_guest_email_verification(self, session: UserSession, email: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if session.email and sanitized_email != session.email: # Used sanitized_email consistently
    ...
    def get_ai_response(self, session: UserSession, prompt: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get AI response (full content and metadata)
            ai_response = self.ai.get_response(sanitized_prompt, session.messages)
            
            # Add messages to session (store the full content)
        ...
        with st.form("email_verification_form", clear_on_submit=False):
            ...
            if submit_email:
                if current_email_input:
                    # The handle_guest_email_verification method now contains the email switch logic,
                    # so we just need to pass the current_email_input.
            ...
            with st.spinner("🔍 Processing your question..."):
                try:
                    ...
                    if response.get('requires_email'):
                        ...
                        # Generator function for character-by-character streaming
                        def stream_content_char_by_char(text: str) -> Generator[str, None, None]:
                            for char in text:
                                yield char
                                time.sleep(0.01) # Adjust this for faster/slower streaming
                        ...
                        indicators = []
                        if response.get("used_pinecone"): indicators.append("🧠 Knowledge Base")
                        if response.get("used_search"): indicators.append("🌐 Web Search")
                        if indicators: st.caption(f"Enhanced with: {', '.join(indicators)}")
                        ...
                        if response.get("has_citations") and response.get("has_inline_citations"):
                            st.caption("📚 Response includes verified citations")
                        # --- END OF STREAMING EFFECT ---

                        logger.info(f"✅ Question processed successfully and response displayed")
