...
# =============================================================================
# VERSION 3.4 PRODUCTION - FIXED SESSION STORAGE & AUTO-SAVE
# - FIXED: Session storage now uses st.session_state for persistence across reruns
# - FIXED: Removed sendBeacon, replaced with image pixel method for browser events
# - ENHANCED: Comprehensive logging and diagnostics for all save operations
# - ENHANCED: Fallback mechanisms for failed session loads
# - ENHANCED: Real-time storage monitoring and validation
# =============================================================================
...
class DatabaseManager:
    def __init__(self, connection_string: Optional[str]):
        ...
        # Try SQLite Cloud first, then fall back to session state storage
        if connection_string and SQLITECLOUD_AVAILABLE:
            ...
            logger.info("SQLite Cloud not available or not configured")
            self._init_local_storage()

    def _init_local_storage(self):
        logger.info("Using Streamlit session state for session storage")
        ...
        with self.lock:
            if self.use_cloud:
                try:
                    ...
                    logger.info(f"Fell back to session state storage for session {session.session_id[:8]}")
            else:
    ...
    def load_session(self, session_id: str) -> Optional[UserSession]:
        with self.lock:
            ...
            # Load from Streamlit session state (fallback or primary)
            ...
            if session:
                ...
                logger.debug(f"Session {session_id[:8]} not found in session state")
                available_sessions = list(st.session_state.fifi_sessions.keys())
                logger.debug(f"Available sessions: {[s[:8] for s in available_sessions]}")
                return None

    def get_storage_info(self) -> Dict[str, Any]:
        """Get diagnostic information about current storage"""
        info = {
            "storage_type": "SQLite Cloud" if self.use_cloud else "Session State",
        ...
        if self.use_cloud:
            try:
                with self._get_connection() as conn:
                    ...
                    cursor = conn.execute("SELECT session_id, email, created_at FROM sessions WHERE active = 1 LIMIT 10")
...
def render_auto_logout_component(timeout_seconds: int, session_id: str, session_manager: SessionManager):
    """Enhanced auto-logout component with reliable pre-timeout save."""
    ...
    (function() {{
        ...
        // Function to trigger the save using reliable methods
        function triggerPreTimeoutSave() {{
            ...
            savingDiv.style.cssText = 'position: fixed; top: 20px; right: 20px; background: #ff6b6b; color: white; padding: 10px 20px; border-radius: 5px; z-index: 9999; font-family: sans-serif;';
            savingDiv.textContent = 'Auto-saving session...';
            document.body.appendChild(savingDiv);

            // Method 1: Image pixel (most reliable)
            const img = new Image();
            img.src = saveUrl;
            img.onload = () => console.log('Pre-timeout save sent via image pixel');
            img.onerror = () => console.log('Image pixel method failed');
            ...
            const fetchPromise = fetch(saveUrl, {{
                ...
                console.log('Pre-timeout save sent via fetch');
            }}).catch(err => {{
                console.error('Fetch method failed:', err);
            }});
            
            return fetchPromise;
        }}
        
        // Function to execute the complete logout sequence
        function executeLogoutSequence() {{
            console.log('Starting logout sequence...');
            
            triggerPreTimeoutSave()
                .catch(err => {{
                    console.error('Pre-timeout save failed:', err);
                }})
                .finally(() => {{
                    console.log('Save request completed. Reloading parent page in 1 second...');
                    setTimeout(() => {{
                ...
                if session_manager:
                    ...
                    if not already_saved:
                        ...
                        if success:
                            ...
                            logger.info("Pre-timeout save successful")
                        else:
                            st.warning("⚠️ Could not save chat before timeout")
                            logger.warning("Pre-timeout save failed")
                    else:
                        st.info("ℹ️ Session already saved during this timeout cycle")
                        logger.info("Pre-timeout save skipped - already completed")
