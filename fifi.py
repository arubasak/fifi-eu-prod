...
from urllib.parse import urlparse, quote_plus # Added quote_plus for URL encoding
...
class Config:
    def __init__(self):
        ...
        # NEW: Streamlit App URL and Beacon Base URL for redirect strategy
        self.STREAMLIT_APP_URL = st.secrets.get("STREAMLIT_APP_URL", "https://your-streamlit-app-url.streamlit.app")
        self.FASTAPI_BEACON_BASE_URL = st.secrets.get("FASTAPI_BEACON_BASE_URL", "https://your-beacon-fastapi-url.run.app")
...
class DatabaseManager:
    ...
    class FingerprintingManager:
        ...
        def __init__(self):
            ...
            self.fingerprint_cache = defaultdict(lambda: {'last_seen': datetime.min}) # Ensure default value is callable
...
class SessionManager:
    ...
    def _create_new_session(self) -> UserSession:
        ...
        # This will be replaced by a real one for Guests, or marked as 'not_collected_registered_user' for Registered Users later.
            ...
            if response.status_code == 200:
                ...
                if wp_token and user_email:
                    ...
                    try:
                        ...
                        ## CHANGE: Set app_stage directly to CHAT for registered user
                        st.session_state.app_stage = 'CHAT'
                        st.session_state.is_chat_ready = True # Ensure chat is ready
                        st.session_state.fingerprint_wait_start = None # Clear any FP wait state
                    except Exception as e:
...
def process_fingerprint_from_query(session_id: str, fingerprint_id: str, method: str, privacy: str, working_methods: List[str]) -> bool:
    ...
    try:
        ...
        if session.user_type == UserType.REGISTERED_USER:
            ...
            st.session_state.is_chat_ready = True # This ensures the session can transition to CHAT stage
            ...
            # After processing, explicitly set app_stage to CHAT for registered user
            st.session_state.app_stage = 'CHAT' 
        ...
        if success:
            ...
            # After successful fingerprint for guest, transition to CHAT stage
            st.session_state.app_stage = 'CHAT'
            
            logger.info(f"Chat input unlocked for session {session.session_id[:8]} after successful JS fingerprinting. Transitioning to CHAT stage.")
            return True
        else:
            ...
            # If fingerprint failed for guest, transition to CHAT stage anyway (with fallback FP)
            st.session_state.app_stage = 'CHAT'
...
def handle_fingerprint_requests_from_query(session_manager: 'SessionManager'):
    ...
    if (event == "fingerprint_complete" or event == "fingerprint_complete_fallback") and session_id:
        ...
        try:
            ...
            fingerprint_data = {
                'fingerprint_id': st.session_state.fingerprint_id_from_js,
                'fingerprint_method': st.session_state.fingerprint_method_from_js,
...
def render_fingerprinting_page(session_manager: 'SessionManager', session: UserSession):
    """
    Renders a dedicated, stable page whose ONLY purpose is to collect the fingerprint.
    It relies on the JS component to send the FP to the Beacon,
    and Streamlit's Python side polls the DB to detect the update.
    This version removes the aggressive st.rerun() loop to prevent flickering.
    """
    st.set_page_config(
...
def render_full_chat_ui(session_manager: 'SessionManager', session: UserSession):
    ...
    This is only called AFTER fingerprinting is complete or bypassed.
    """
...
def render_welcome_page(session_manager: 'SessionManager'):
    ...
    with col3:
        ...
        st.markdown(f"  - **Tier 2**: Questions {REGISTERED_USER_TIER_1_LIMIT + 1}-{REGISTERED_USER_QUESTION_LIMIT} → {TIER_2_BAN_HOURS}-hour reset")
        ...
        if session.user_type.value == UserType.REGISTERED_USER.value:
            ...
            if session.daily_question_count < REGISTERED_USER_TIER_1_LIMIT:
                ...
                if session.ban_status == BanStatus.ONE_HOUR and session.ban_end_time and datetime.now() < session.ban_end_time:
                    ...
                    st.caption(f"🚫 {TIER_1_BAN_HOURS}-hour break required to access Tier 2")
                else:
                    st.progress(1.0, text="Tier 1 Complete ✅")
            ...
            if session.ban_status == BanStatus.TWENTY_FOUR_HOUR and session.ban_end_time and datetime.now() < session.ban_end_time:
                ...
                next_reset = session.last_question_time + timedelta(hours=EMAIL_VERIFIED_BAN_HOURS)
...
def display_email_prompt_if_needed(session_manager: 'SessionManager', session: UserSession) -> bool:
    ...
    # Handle the actual email sending if flagged
    ...
    is_guest_limit_hit = (user_is_guest and session.daily_question_count >= GUEST_QUESTION_LIMIT)
    ...
    # PRIORITY 2: Handle guest who has hit their limit
    ...
    # (The rest of the logic for different stages of the email prompt)
    if current_stage == 'initial_check':
        ...
        st.info(f"Please verify **{masked_email}** to reclaim your status.")
        ...
        with col1:
            if st.button("✅ Verify this email", use_container_width=True):
                ...
                st.rerun()
        with col2:
            if st.button("❌ No, use a different email", use_container_width=True):
                session.recognition_response = "no_declined_reco"
                session.declined_recognized_email_at = datetime.now()
                session.reverification_pending = False # Reset pending state
                session_manager.db.save_session(session)
                st.session_state.verification_stage = 'email_entry'
                st.rerun()
        ...
        email_to_verify = st.session_state.get('verification_email', session.email)
        st.success(f"📧 **Verification code sent to** **{session_manager._mask_email(email_to_verify)}**")
...
def main_fixed():
    ...
    # Handle fingerprint data and emergency saves from URL query params
    # This must run early to catch redirects and stop further execution if needed.
    session_manager = st.session_state.get('session_manager')
    if not session_manager:
