...
try:
    ...
    logger = logging.getLogger(__name__)
    logger.warning(
        "⚠️ streamlit_js_eval not available, using fallback timeout mechanism")
...
# Graceful fallbacks for optional imports (from B)
...
try:
    # MODIFIED: Import TavilySearch instead of TavilyClient for Langchain integration (from B)
...
try:
    ...
    logger.warning(
        "Supabase SDK not available. Email verification will be disabled.")

# Competition exclusion list for web searches (from B)
...
# Utility for safe JSON loading (from B)
...
# =============================================================================
# LOADING STATE MANAGEMENT (NEW) (from B)
# =============================================================================
...
def show_loading_overlay():
    ...
    if is_loading():
        loading_message = st.session_state.get(
...
class Config:
    def __init__(self):
        ...
        self.PINECONE_ASSISTANT_NAME = st.secrets.get(
            "PINECONE_ASSISTANT_NAME", "my-chat-assistant")
        self.WORDPRESS_URL = self._validate_url(
            st.secrets.get("WORDPRESS_URL", ""))
        self.SQLITE_CLOUD_CONNECTION = st.secrets.get(
            "SQLITE_CLOUD_CONNECTION")
        self.ZOHO_CLIENT_ID = st.secrets.get("ZOHO_CLIENT_ID")
        self.ZOHO_CLIENT_SECRET = st.secrets.get("ZOHO_CLIENT_SECRET")
        self.ZOHO_REFRESH_TOKEN = st.secrets.get("ZOHO_REFRESH_TOKEN")
        self.ZOHO_ENABLED = all(
        ...
        self.SUPABASE_ENABLED = all(
            [SUPABASE_AVAILABLE, self.SUPABASE_URL, self.SUPABASE_ANON_KEY])

    def _validate_url(self, url: str) -> str:
        if url and not url.startswith(('http://', 'https://')):
            logger.warning(
                f"Invalid URL format for WORDPRESS_URL: {url}. Disabling feature.")
            return ""
        return url.rstrip('/')

# =============================================================================
# ERROR HANDLING SYSTEM (from B)
# =============================================================================
    ...
    def handle_api_error(self, component: str, operation: str, error: Exception) -> ErrorContext:
        ...
        return ErrorContext(
            ...
            recovery_suggestions=[
                "Try again", "Check your internet", "Contact support if issue persists"],
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                error_context = error_handler.handle_api_error(
                    component, operation, e)
                error_handler.log_error(error_context)
                if show_to_user:
                    error_handler.display_error_to_user(error_context)
                logger.error(
                    f"API Error in {component}/{operation}: {e}", exc_info=True)
                return None
        return wrapper
    return decorator
...
class DatabaseManager:  # From A
    def __init__(self, connection_string: Optional[str]):
        ...
        if not self.conn:
            logger.critical(
                "🚨 ALL DATABASE CONNECTIONS FAILED. FALLING BACK TO NON-PERSISTENT IN-MEMORY STORAGE.")
            self.db_type = "memory"
            self.local_sessions = {}
    ...
    def _try_local_sqlite(self):
        try:
            conn = sqlite3.connect("fifi_sessions_v2.db",
        ...
        try:
            ...
            for col_name, col_type in new_columns:
                try:
                    ...
                    logger.debug(
                        f"✅ Added {col_name} column to existing database")
                except Exception as alter_error:
                    logger.debug(
                        f"ALTER TABLE for {col_name} failed (likely already exists): {alter_error}")

            # Create essential indexes
            self.conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_session_lookup ON sessions(session_id, active)")
            self.conn.execute(
    ...
    def _ensure_connection_healthy(self, config_instance: Any):
        ...
        if not self._check_connection_health():
            logger.warning(
                "Database connection unhealthy, attempting reconnection...")
            old_conn = self.conn
            self.conn = None
            ...
            for attempt in range(max_retries):
                try:
                    if self.db_type == "cloud" and SQLITECLOUD_AVAILABLE and hasattr(config_instance, 'SQLITE_CLOUD_CONNECTION'):
                        self.conn, _ = self._try_sqlite_cloud(
                            config_instance.SQLITE_CLOUD_CONNECTION)
                    ...
                    if self.conn:
                        self.conn.execute("SELECT 1").fetchone()
                        logger.info(
                            f"✅ Database reconnection successful on attempt {attempt + 1}")
                        return

                except Exception as e:
                    logger.error(
                        f"Database reconnection attempt {attempt + 1} failed: {e}")
                    if attempt < max_retries - 1:
                        time.sleep(1)

            logger.error(
                "All database reconnection attempts failed, falling back to in-memory storage")
            self.db_type = "memory"
            if not hasattr(self, 'local_sessions'):
                self.local_sessions = {}

    @handle_api_errors("Database", "Save Session")
    def save_session(self, session: UserSession):  # From A
        """Save session with SQLite Cloud compatibility and connection health check"""
        logger.debug(
            f"💾 SAVING SESSION TO DB: {session.session_id[:8]} | user_type={session.user_type.value} | email={session.email} | messages={len(session.messages)} | daily_q={session.daily_question_count} | fp_id={session.fingerprint_id[:8]} | active={session.active}")
        # with self.lock: # REMOVED THIS LINE
        # Check and ensure connection health before any DB operation
        current_config = st.session_state.get('session_manager').config if st.session_state.get(
            'session_manager') else None
        if current_config:
        ...
        if self.db_type == "memory":
            ...
            logger.debug(
                f"Saved session {session.session_id[:8]} to in-memory.")
            return
        ...
        try:
            ...
            if not isinstance(session.messages, list):
                logger.warning(
                    f"Invalid messages field for session {session.session_id[:8]}, resetting to empty list")
                session.messages = []
            ...
            try:
                ...
                logger.error(
                    f"Session data not JSON serializable for {session.session_id[:8]}: {e}. Resetting data to empty lists.")
                json_messages = "[]"
                json_emails_used = "[]"
                session.messages = []
            ...
            declined_recognized_email_at_iso = session.declined_recognized_email_at.isoformat(
            ) if session.declined_recognized_email_at else None
            ...
            self.conn.execute(
                ...
                (session.session_id, session.user_type.value, session.email, session.full_name,
                 ...
                 # This is the corrected line for fingerprint_method
                 session.fingerprint_method, session.visitor_type,
                 session.daily_question_count,
                 session.total_question_count,
                 session.last_question_time.isoformat(
                 ) if session.last_question_time else None,
                 int(session.question_limit_reached), session.ban_status.value,
                 session.ban_start_time.isoformat(
                 ) if session.ban_start_time else None,
                 session.ban_end_time.isoformat(
                 ) if session.ban_end_time else None,
                 session.ban_reason, session.evasion_count, session.current_penalty_hours,
                 session.escalation_level, json_emails_used,
                 session.email_switches_count, session.browser_privacy_level, int(
                     session.registration_prompted),
            ...
            logger.debug(
                f"Successfully saved session {session.session_id[:8]}: user_type={session.user_type.value}, active={session.active}, rev_pending={session.reverification_pending}")

        except Exception as e:
            logger.error(
                f"Failed to save session {session.session_id[:8]}: {e}", exc_info=True)
            # Try to fallback to in-memory on save failure
            if not hasattr(self, 'local_sessions'):
                self.local_sessions = {}
            self.local_sessions[session.session_id] = copy.deepcopy(session)
            logger.info(
                f"Fallback: Saved session {session.session_id[:8]} to in-memory storage")
            raise

    @handle_api_errors("Database", "Load Session")
    def load_session(self, session_id: str) -> Optional[UserSession]:  # From A
        ...
        if self.db_type == "memory":
            ...
            if session and not hasattr(session, 'reverification_pending'):
                ...
                logger.debug(
                    f"Added missing re-verification fields to in-memory session {session_id[:8]}")
        ...
        try:
            ...
            if not row:
                logger.debug(
                    f"No active session found for ID {session_id[:8]}.")
                return None

            # Handle as tuple (SQLite Cloud returns tuples)
            # Expected 32 columns before adding re-verification fields. Now 38. Now 39.
            # Updated expected columns based on previous version without `pending_daily_question_count`
            expected_min_cols = 39
            if len(row) < expected_min_cols:  # Must have at least this many columns for full functionality
                logger.error(
                    f"Row has insufficient columns: {len(row)} (expected at least {expected_min_cols}). Data corruption suspected or old schema.")
            ...
            try:
                ...
                loaded_display_message_offset = row[31] if len(
                    row) > 31 else 0

                # NEW: Safely get re-verification fields, defaulting if columns are missing
                loaded_reverification_pending = bool(
                    row[32]) if len(row) > 32 else False
                loaded_pending_user_type = UserType(
                    row[33]) if len(row) > 33 and row[33] else None
                loaded_pending_email = row[34] if len(row) > 34 else None
                loaded_pending_full_name = row[35] if len(row) > 35 else None
                loaded_pending_zoho_contact_id = row[36] if len(
                    row) > 36 else None
                loaded_pending_wp_token = row[37] if len(row) > 37 else None
                # NEW: Safely get declined_recognized_email_at
                loaded_declined_recognized_email_at = datetime.fromisoformat(
                    row[38]) if len(row) > 38 and row[38] else None

                # Convert last_activity from ISO format string or None
                loaded_last_activity = datetime.fromisoformat(
                ...
                user_session = UserSession(
                    ...
                    # This line loads the *old* value from DB
                    visitor_type=row[13] or 'new_visitor',
                    daily_question_count=row[14] or 0,
                    total_question_count=row[15] or 0,
                    last_question_time=datetime.fromisoformat(
                        row[16]) if row[16] else None,
                    question_limit_reached=bool(row[17]),
                    ban_status=BanStatus(
                        row[18]) if row[18] else BanStatus.NONE,
                    ban_start_time=datetime.fromisoformat(
                        row[19]) if row[19] else None,
                    ban_end_time=datetime.fromisoformat(
                        row[20]) if row[20] else None,
                    ban_reason=row[21],
                    evasion_count=row[22] or 0,
                    current_penalty_hours=row[23] or 0,
                    escalation_level=row[24] or 0,
                    email_addresses_used=safe_json_loads(
                        row[25], default_value=[]),
                ...
                logger.debug(
                    f"Successfully loaded session {session_id[:8]}: user_type={user_session.user_type.value}, messages={len(user_session.messages)}, active={user_session.active}, rev_pending={user_session.reverification_pending}")
                return user_session

            except Exception as e:
                logger.error(
                    f"Failed to create UserSession object from row for session {session_id[:8]}: {e}", exc_info=True)
                logger.error(
                    f"Problematic row data (truncated): {str(row)[:200]}")
                return None

        except Exception as e:
            logger.error(
                f"Failed to load session {session_id[:8]}: {e}", exc_info=True)
            return None

    @handle_api_errors("Database", "Find by Fingerprint")
    def find_sessions_by_fingerprint(self, fingerprint_id: str) -> List[UserSession]:  # From A
        ...
        if self.db_type == "memory":
            sessions = [copy.deepcopy(s) for s in self.local_sessions.values(
            ...
            for session in sessions:
                ...
                if not hasattr(session, 'declined_recognized_email_at'):  # NEW field
                    session.declined_recognized_email_at = None
            logger.debug(
                f"📊 FINGERPRINT SEARCH RESULTS (MEMORY): Found {len(sessions)} sessions for {fingerprint_id[:8]}")
            return sessions
        ...
        try:
            ...
            for row in cursor.fetchall():
                ...
                if len(row) < expected_min_cols:  # Must have at least this many columns for full functionality
                    logger.warning(
                        f"Row has insufficient columns in find_sessions_by_fingerprint: {len(row)} (expected at least {expected_min_cols}). Skipping row.")
                    continue
                try:
                    # Safely get display_message_offset, defaulting to 0 if column is missing (backward compatibility)
                    loaded_display_message_offset = row[31] if len(
                    ...
                    loaded_pending_user_type = UserType(
                        row[33]) if len(row) > 33 and row[33] else None
                    loaded_pending_email = row[34] if len(row) > 34 else None
                    loaded_pending_full_name = row[35] if len(
                        row) > 35 else None
                    loaded_pending_zoho_contact_id = row[36] if len(
                        row) > 36 else None
                    loaded_pending_wp_token = row[37] if len(
                        row) > 37 else None
                    # NEW: Safely get declined_recognized_email_at
                    loaded_declined_recognized_email_at = datetime.fromisoformat(
                        row[38]) if len(row) > 38 and row[38] else None
                    ...
                    s = UserSession(
                        session_id=row[0],
                        user_type=UserType(
                            row[1]) if row[1] else UserType.GUEST,
                        email=row[2],
                        full_name=row[3],
                        zoho_contact_id=row[4],
                        created_at=datetime.fromisoformat(
                    ...
                    logger.error(
                        f"Error converting row to UserSession in find_sessions_by_fingerprint: {e}", exc_info=True)
                    continue
            logger.debug(
                f"📊 FINGERPRINT SEARCH RESULTS (DB): Found {len(sessions)} sessions for {fingerprint_id[:8]}")
            for s in sessions:
                logger.debug(
                    f"  - {s.session_id[:8]}: type={s.user_type.value}, email={s.email}, daily_q={s.daily_question_count}, total_q={s.total_question_count}, last_activity={s.last_activity}, active={s.active}, rev_pending={s.reverification_pending}")
            return sessions
        except Exception as e:
            logger.error(
                f"Failed to find sessions by fingerprint '{fingerprint_id[:8]}...': {e}", exc_info=True)
            return []
    ...
    class FingerprintingManager:  # From A
        ...
        def render_fingerprint_component(self, session_id: str):
            ...
            try:
                ...
                html_file_path = os.path.join(
                    current_dir, 'fingerprint_component.html')

                logger.debug(
                    f"🔍 Looking for fingerprint component at: {html_file_path}")

                if not os.path.exists(html_file_path):
                    logger.error(
                        f"❌ Fingerprint component file NOT FOUND at {html_file_path}")
                    logger.info(
                        f"📁 Current directory contents: {os.listdir(current_dir)}")
                    return self._generate_fallback_fingerprint()

                logger.debug(
                    f"✅ Fingerprint component file found, reading content...")

                with open(html_file_path, 'r', encoding='utf-8') as f:
                    html_content = f.read()

                logger.debug(
                    f"📄 Read {len(html_content)} characters from fingerprint component file")
                ...
                if original_content == html_content:
                    logger.warning(
                        f"⚠️ No {{SESSION_ID}} placeholder found in HTML content!")
                else:
                    logger.debug(
                        f"✅ Replaced {{SESSION_ID}} placeholder with {session_id[:8]}...")

                # Render with minimal visibility (height=0 for silent operation)
                logger.debug(
                    f"🔄 Rendering fingerprint component for session {session_id[:8]}...")
                # REMOVE 'key' ARGUMENT HERE
                st.components.v1.html(
                    html_content, height=0, width=0, scrolling=False)

                logger.info(
                    f"✅ External fingerprint component rendered successfully for session {session_id[:8]}")
                return None  # Always return None since data comes via redirect

            except Exception as e:
                logger.error(
                    f"❌ Failed to render external fingerprint component: {e}", exc_info=True)
                return self._generate_fallback_fingerprint()
        ...
        def process_fingerprint_data(self, fingerprint_data: Dict[str, Any]) -> Dict[str, Any]:
            ...
            if not fingerprint_data or fingerprint_data.get('error'):
                logger.warning(
                    "Fingerprint component returned error. Using fallback.")
                return self._generate_fallback_fingerprint()
            ...
            if not fingerprint_id or not fingerprint_method:
                logger.warning(
                    "Invalid fingerprint data received. Using fallback.")
                return self._generate_fallback_fingerprint()
            ...
            return {
                ...
                # <-- This is where process_fingerprint_data sets visitor_type
    ...
    class EmailVerificationManager:  # From B
        ...
        def __init__(self, config: Config):
            ...
            if self.config.SUPABASE_ENABLED:
                try:
                    ...
                    logger.info(
                        "✅ Supabase client initialized for email verification.")
                except Exception as e:
                    logger.error(
                        f"❌ Supabase client initialization failed: {e}. Email verification will be disabled.")
                    self.supabase = None
        ...
        def send_verification_code(self, email: str) -> bool:
            if not self.supabase:
                st.error(
                    "Email verification service is not available (Supabase not configured/failed).")
                return False
            ...
            try:
                ...
                if response is not None:
                    logger.info(
                        f"Email OTP code sent to {email} via Supabase.")
                    return True
                else:
                    logger.error(
                        f"Supabase OTP send failed - unexpected response from sign_in_with_otp.")
                    return False

            except Exception as e:
                logger.error(
                    f"Failed to send verification code via Supabase for {email}: {e}")
        ...
        def verify_code(self, email: str, code: str) -> bool:
            ...
            try:
                ...
                if response and response.user:
                    logger.info(
                        f"Email verification successful for {email} (Supabase User ID: {response.user.id}).")
                    return True
                else:
                    logger.warning(
                        f"Email verification failed for {email}: Invalid code or no user returned.")
                    st.error(
                ...
                logger.error(
                    f"Failed to verify code via Supabase for {email}: {e}")
                st.error(f"Verification failed: {str(e)}")
                return False

    class QuestionLimitManager:  # From B
        ...
        def __init__(self):
            ...
            self.evasion_penalties = [
                24, 48, 96, 192, 336]  # Escalating penalties in hours
        ...
        def detect_guest_email_evasion(self, session: UserSession, db_manager) -> bool:
            ...
            try:
                ...
                if (session.declined_recognized_email_at and
                    ...
                    logger.info(
                        f"Evasion check bypassed for {session.session_id[:8]}: Just declined recognized email and within guest limit.")
                    return False
                ...
                if session.declined_recognized_email_at and \
                   session.daily_question_count >= self.question_limits[UserType.GUEST.value]:
                    logger.info(
                        f"Evasion grace period ended for {session.session_id[:8]}: Guest limit reached.")
                ...
                fingerprint_sessions = db_manager.find_sessions_by_fingerprint(
                ...
                for fp_session in fingerprint_sessions:
                    if (fp_session.session_id != session.session_id and
                        # Use .value for robustness
                        fp_session.user_type.value == UserType.GUEST.value and
                        ...
                        # Only check if the limit was hit and 24 hours have NOT passed
                ...
                if recent_limit_sessions:
                    logger.warning(
                        f"🚨 EVASION DETECTED: Session {session.session_id[:8]} switching emails after hitting guest limit. Found {len(recent_limit_sessions)} recent limited sessions.")
                    return True
                ...
                logger.error(
                    f"Error in evasion detection for session {session.session_id[:8]}: {e}")
                return False
        ...
        def is_within_limits(self, session: UserSession) -> Dict[str, Any]:
            ...
            if session.ban_status.value != BanStatus.NONE.value:
                if session.ban_end_time and datetime.now() < session.ban_end_time:
                    ...
                    logger.info(
                        f"Ban for session {session.session_id[:8]} expired. Resetting status and counters.")
            ...
            if session.last_question_time:
                ...
                if time_since_last >= timedelta(hours=24):
                    logger.info(
                        f"Daily question count reset for session {session.session_id[:8]} due to 24-hour window expiration.")
                    session.daily_question_count = 0
                    session.question_limit_reached = False
            ...
            if session.user_type.value == UserType.REGISTERED_USER.value:
                if session.daily_question_count >= user_limit:  # For registered, user_limit is 20
                    return {
                        'allowed': False,
                        # New specific reason
                        'reason': 'registered_user_tier2_limit',
                ...
                # If at 10 or more, but less than 20 (user_limit)
                    ...
                    if session.daily_question_count == 10:
                        return {
                            'allowed': False,
                            # New specific reason
                            'reason': 'registered_user_tier1_limit',
            ...
            if session.user_type.value == UserType.GUEST.value:
                ...
                if session.daily_question_count >= user_limit:
                    return {
                        'allowed': False,
                        # New specific reason
                        'reason': 'email_verified_guest_limit',
        ...
        def record_question(self, session: UserSession):
            ...
            logger.debug(
                f"Question recorded for {session.session_id[:8]}: daily={session.daily_question_count}, total={session.total_question_count}")
        ...
        def apply_evasion_penalty(self, session: UserSession) -> int:
            ...
            session.escalation_level = min(
                session.evasion_count, len(self.evasion_penalties))

            penalty_hours = self.evasion_penalties[session.escalation_level - 1]
            session.current_penalty_hours = penalty_hours

            self._apply_ban(
                session, BanStatus.EVASION_BLOCK, f"Email switching evasion attempt #{session.evasion_count}")

            logger.warning(
                f"🚨 EVASION PENALTY: {penalty_hours}h ban applied to {session.session_id[:8]} (Level {session.escalation_level})")
        ...
        def _apply_ban(self, session: UserSession, ban_type: BanStatus, reason: str):
            ...
            session.ban_end_time = session.ban_start_time + \
                timedelta(hours=ban_hours)
            session.ban_reason = reason
            session.question_limit_reached = True

            logger.info(
                f"Ban applied to session {session.session_id[:8]}: Type={ban_type.value}, Duration={ban_hours}h, Reason='{reason}'.")
...
# =============================================================================
# PDF EXPORTER & ZOHO CRM MANAGER (MOVED OUT OF DATABASEMANAGER) (from B)
# =============================================================================


class PDFExporter:  # From B
    ...
    def __init__(self):
        ...
        # Increased line spacing (1.4 * font size)
    ...
    def generate_chat_pdf(self, session: UserSession) -> Optional[io.BytesIO]:
        ...
        story = []
        story.append(Paragraph("FiFi AI Chat Transcript",
                     self.styles['ChatHeader']))
        ...
        for msg in session.messages:
            ...
            if msg.get("source"):
                story.append(
                    Paragraph(f"<i>Source: {msg['source']}</i>", self.styles['Caption']))

            # Add a bit more space between messages if not handled by spaceAfter
            # Only add if the style doesn't already define spaceAfter
...
class ZohoCRMManager:  # From B
    ...
    def _get_access_token(self, force_refresh: bool = False, timeout: int = 15) -> Optional[str]:
        ...
        if not self.config.ZOHO_ENABLED:
            logger.debug(
                "Zoho is not enabled in configuration. Skipping token request.")
            return None
        ...
        try:
            logger.info(
                f"Requesting new Zoho access token with {timeout}s timeout...")
            ...
            logger.error(
                f"Failed to get Zoho access token: {e}", exc_info=True)
            return None
    ...
    def _find_contact_by_email(self, email: str) -> Optional[str]:
        ...
        try:
            response = requests.get(
                f"{self.base_url}/Contacts/search", headers=headers, params=params, timeout=10)

            if response.status_code == 401:
                logger.warning(
                    "Zoho token expired, attempting refresh for _find_contact_by_email...")
            ...
            logger.error(
                f"Error finding Zoho contact by email {email}: {e}", exc_info=True)
    ...
    def _create_contact(self, email: str, full_name: Optional[str]) -> Optional[str]:
        ...
        try:
            response = requests.post(
                f"{self.base_url}/Contacts", headers=headers, json=contact_data, timeout=10)

            if response.status_code == 401:
                logger.warning(
                    "Zoho token expired, attempting refresh for _create_contact...")
                access_token = self._get_access_token(force_refresh=True)
                if not access_token:
                    return None
                headers['Authorization'] = f'Zoho-oauthtoken {access_token}'
            ...
            logger.error(
                f"Error creating Zoho contact for {email}: {e}", exc_info=True)
        ...
        for attempt in range(max_retries):
            try:
                ...
                response = requests.post(
                    upload_url,
                    headers=headers,
                    files={'file': (filename, pdf_buffer.read(),
                                   'application/pdf')},
                    timeout=60
                )
                ...
                if response.status_code == 401:
                    logger.warning(
                        "Zoho token expired during upload, attempting refresh...")
                    access_token = self._get_access_token(force_refresh=True)
                    if not access_token:
                        return False
                    headers['Authorization'] = f'Zoho-oauthtoken {access_token}'
                ...
                if 'data' in data and data['data'][0]['code'] == 'SUCCESS':
                    ...
                    logger.error(
                        f"Zoho attachment upload failed with response: {data}")

            except requests.exceptions.Timeout:
                logger.error(
                    f"Zoho upload timeout (attempt {attempt + 1}/{max_retries})")
            except Exception as e:
                logger.error(
                    f"Error uploading Zoho attachment (attempt {attempt + 1}/{max_retries}): {e}", exc_info=True)
    ...
    def _add_note(self, contact_id: str, note_title: str, note_content: str) -> bool:
        ...
        for attempt in range(max_retries):
            try:
                response = requests.post(
                    f"{self.base_url}/Notes", headers=headers, json=note_data, timeout=10)

                if response.status_code == 401:
                    logger.warning(
                        "Zoho token expired during note add, attempting refresh...")
                    access_token = self._get_access_token(force_refresh=True)
                    if not access_token:
                        return False
                    headers['Authorization'] = f'Zoho-oauthtoken {access_token}'
                ...
                if 'data' in data and data['data'][0]['code'] == 'SUCCESS':
                    logger.info(
                        f"Successfully added note to Zoho contact {contact_id}")
                    return True
                else:
                    ...
                    if attempt < (2 - 1):
                        ...
                        logger.error(
                            "Max retries for note addition reached. Aborting save.")
                        return False
            except Exception as e:
                logger.error(
                    f"ZOHO NOTE ADD FAILED on attempt {attempt + 1} with an exception.")
                logger.error(
    ...
    def save_chat_transcript_sync(self, session: UserSession, trigger_reason: str) -> bool:
        ...
        if (session.user_type.value not in [UserType.REGISTERED_USER.value, UserType.EMAIL_VERIFIED_GUEST.value] or
            ...
            logger.info(
                f"ZOHO SAVE SKIPPED: Not eligible. (UserType: {session.user_type.value}, Email: {bool(session.email)}, Messages: {bool(session.messages)}, Zoho Enabled: {self.config.ZOHO_ENABLED})")
            return False
        ...
        if not contact_id:
            logger.error(
                "Failed to find or create Zoho contact. Cannot proceed with save.")
            return False
        ...
        if pdf_buffer:
            ...
            upload_success = self._upload_attachment(
                contact_id, pdf_buffer, pdf_filename)
            if not upload_success:
                logger.warning(
                    "Failed to upload PDF attachment to Zoho. Continuing with note only.")

        # Step 3: Add note with retry logic
        max_retries_note = 3 if "timeout" in trigger_reason.lower(
        ) or "emergency" in trigger_reason.lower() else 1
        for attempt_note in range(max_retries_note):
            try:
                ...
                note_content = self._generate_note_content(
                    session, upload_success, trigger_reason)
                note_success = self._add_note(
                    contact_id, note_title, note_content)

                if note_success:
                    logger.info("=" * 80)
                    logger.info(
                        f"ZOHO SAVE COMPLETED SUCCESSFULLY on note attempt {attempt_note + 1}")
                ...
                logger.error(
                    f"ZOHO NOTE ADD FAILED on attempt {attempt_note + 1} with an exception.")
                logger.error(
                    f"Error: {type(e).__name__}: {str(e)}", exc_info=True)
                if attempt_note < max_retries_note - 1:
                    time.sleep(2 ** attempt_note)
    ...
    def is_allowed(self, identifier: str) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            self.requests[identifier] = [
                t for t in self.requests[identifier] if t > now - self.window_seconds]

            if len(self.requests[identifier]) < self.max_requests:
                self.requests[identifier].append(now)
                logger.debug(
                    f"Rate limit allowed for {identifier[:8]}... ({len(self.requests[identifier])}/{self.max_requests} within {self.window_seconds}s)")
                return {
                    'allowed': True,
                ...
                time_until_next = max(
                    0, int((oldest_request + self.window_seconds) - now))

                logger.warning(
                    f"Rate limit exceeded for {identifier[:8]}... ({len(self.requests[identifier])}/{self.max_requests} within {self.window_seconds}s)")
                return {
                    'allowed': False,
                    'current_count': len(self.requests[identifier]),
                    'max_requests': self.max_requests,
...
def sanitize_input(text: str, max_length: int = 4000) -> str:  # From B
...
# =============================================================================
# PINECONE ASSISTANT TOOL (from B)
# =============================================================================


class PineconeAssistantTool:  # From B
    ...
    # CHANGE 3: Pinecone Assistant with Business Rules (from B)
    def initialize_assistant(self):
        try:
            ...
            if self.assistant_name not in [a.name for a in assistants_list]:
                logger.warning(
                    f"Assistant '{self.assistant_name}' not found. Creating...")
            ...
            pinecone_messages = [
                PineconeMessage(
                    role="user" if isinstance(
                        msg, HumanMessage) else "assistant",
                    content=msg.content
                ) for msg in chat_history
            ]

            response = self.assistant.chat(
                messages=pinecone_messages, model="gpt-4o")
                    ...
                    for reference in citation.references:
                        if hasattr(reference, 'file') and reference.file:
                            ...
                            if hasattr(reference.file, 'metadata') and reference.file.metadata:
                                link_url = reference.file.metadata.get(
                                    'source_url')
                            ...
                            if link_url:
                                ...
                                display_text = getattr(
                                    reference.file, 'name', 'Unknown Source')
                                if display_text not in seen_items:
...
# CHANGE: LLM-Powered Query Reformulation (Replaces old TavilyFallbackAgent entirely) (from B)


class TavilyFallbackAgent:  # From B
    def __init__(self, tavily_api_key: str, openai_api_key: str = None):
        from tavily import TavilyClient
        self.tavily_client = TavilyClient(api_key=tavily_api_key)
        self.tavily_tool = TavilySearch(
            max_results=5, api_key=tavily_api_key, include_answer=True)  # Keep both for now to avoid breaking existing calls
        ...
        if openai_api_key:
            try:
                ...
                logger.info(
                    "✅ OpenAI client initialized for query reformulation")
            except Exception as e:
                logger.error(
                    f"Failed to initialize OpenAI client for reformulation: {e}")
                self.openai_client = None
    ...
    def reformulate_query_for_search(self, current_question: str, conversation_history: List[BaseMessage]) -> str:
        ...
        try:
            ...
            if len(current_question.split()) > 10 and not any(indicator in current_question.lower()
                ...
                logger.debug(
                    f"Query is detailed enough, using as-is: {current_question}")
                return current_question

            # If no OpenAI client available, fallback to code-based
            if not self.openai_client:
                logger.warning(
                    "OpenAI client not available, using code-based reformulation fallback")
            ...
            if conversation_history:
                ...
                recent_messages = conversation_history[-6:] if len(
                    conversation_history) > 6 else conversation_history
                for msg in recent_messages:
                    if hasattr(msg, 'content') and msg.content:
                        # Truncate long messages to save tokens
                        content = msg.content[:120] + \
                            "..." if len(msg.content) > 120 else msg.content
                        role = "User" if isinstance(
                            msg, HumanMessage) else "Assistant"
                        context_parts.append(f"{role}: {content}")

            conversation_context = "\n".join(
                context_parts) if context_parts else "No prior conversation"
...
OUTPUT: Only the optimized search query, nothing else."""
            ...
            response = self.openai_client.chat.completions.create(
                ...
                messages=[
                    {"role": "system",
                        "content": "You are a search query optimizer. Respond only with the optimized search query."},
            ...
            if not reformulated or len(reformulated) < 3:
                logger.warning(
                    f"LLM returned invalid reformulation: '{reformulated}', using fallback")
                return self._fallback_reformulation(current_question, conversation_history)

            # Ensure query isn't too long (waste of tokens/poor search results)
            if len(reformulated.split()) > 15 or len(reformulated) > 120:
                logger.warning(
                    f"LLM reformulation too long: '{reformulated}', using fallback")
                return self._fallback_reformulation(current_question, conversation_history)

            # Success case
            logger.info(
                f"✅ LLM reformulated: '{current_question}' → '{reformulated}'")
            return reformulated

        except Exception as e:
            logger.error(
                f"LLM reformulation failed: {e}, using code fallback")
            return self._fallback_reformulation(current_question, conversation_history)
            ...
            for msg in conversation_history[-3:]:
                if hasattr(msg, 'content'):
                    ...
                    context_keywords.extend(
                        words[:2])  # Limit to avoid over-expansion
    ...
    def synthesize_search_results(self, results, query: str) -> str:
        ...
        if isinstance(results, dict):
            ...
            # Fallback: extract results array if no answer
                ...
                logger.warning(
                    f"Unexpected dict format from Tavily: {results.keys()}")
        ...
        if not isinstance(results, list):
            logger.warning(
                f"Tavily returned unexpected results type: {type(results)}")
        ...
        response_parts = [
            f"Here is a summary of web search results for '{query}':\n"]
    ...
    def determine_search_strategy(self, question: str, pinecone_error_type: str = None) -> Dict[str, Any]:  # From B (simplified strategy, uses worldwide with exclusions)
    ...
    def query(self, message: str, chat_history: List[BaseMessage], pinecone_error_type: str = None) -> Dict[str, Any]:
        try:
            ...
            standalone_query = self.reformulate_query_for_search(
                message, chat_history)

            # STEP 2: Determine search strategy
            strategy = self.determine_search_strategy(
                standalone_query, pinecone_error_type)
            ...
            if strategy.get("exclude_domains"):
                ...
                logger.info(
                    f"🌐 Tavily worldwide search excluding {len(strategy['exclude_domains'])} competitor domains")
            else:
                logger.info(
                    f"🌐 Tavily worldwide search with reformulated query: {standalone_query}")

            # STEP 4: Execute search with standalone query
            search_results = self.tavily_tool.invoke(input=search_params)
            synthesized_content = self.synthesize_search_results(
                search_results, standalone_query)
...
class EnhancedAI:  # From B
    ...
    def __init__(self, config: Config):
        ...
        if PINECONE_AVAILABLE and self.config.PINECONE_API_KEY and self.config.PINECONE_ASSISTANT_NAME:
            try:
                ...
                self.pinecone_tool = None
                error_handler.log_error(error_handler.handle_api_error(
                    "Pinecone", "Initialize", e))
        ...
        if TAVILY_AVAILABLE and self.config.TAVILY_API_KEY:
            try:
                ...
                logger.info(
                    "✅ Tavily Web Search initialized successfully with LLM reformulation")
                error_handler.mark_component_healthy("Tavily")
            except Exception as e:
                logger.error(f"Tavily agent initialization failed: {e}")
                self.tavily_agent = None
                error_handler.log_error(
                    error_handler.handle_api_error("Tavily", "Initialize", e))
    ...
    def _detect_pinecone_error_type(self, error: Exception) -> str:
        ...
        if any(code in error_str for code in ['401', '403']):
            ...
            return "server_error"
        # FIXED: Changed 'code' to 'keyword' to match other lines
    ...
    def _should_use_pinecone_first(self) -> bool:
        ...
        pinecone_status = error_handler.component_status.get(
            "Pinecone", "healthy")
    ...
    # CHANGE 5: Updated Fallback Logic for Business Rules (from B)
    def should_use_web_fallback(self, pinecone_response: Dict[str, Any], original_question: str) -> bool:
        ...
        logger.warning(
            f"   Response source: {pinecone_response.get('source', 'Unknown')}")
        logger.warning(
            f"   Has citations flag: {pinecone_response.get('has_citations', False)}")
        logger.warning(
            f"   Success flag: {pinecone_response.get('success', False)}")
        logger.warning("=" * 50)
        ...
        if has_citation_markers:
            logger.warning(
                f"🔍 CITATION DEBUG: Found markers in content, has_citations={has_citations_flag}")
            logger.warning(f"Content preview: {content[:200]}...")

        # NEW: Check original question for recency indicators
        recency_indicators = ["latest", "newest",
                              "recent", "current", "2024", "2025"]
        if any(indicator in original_lower for indicator in recency_indicators):
            logger.warning(
                "🔄 Fallback TRIGGERED: User requested latest/current information.")
            return True
        ...
        if "sales-eu@12taste.com" in content or "contact our sales team" in content:
            logger.warning(
                "✅ Fallback SKIPPED: Pinecone provided the correct business redirect for pricing/stock.")
            return False
        logger.warning(
            "✅ Business redirect check PASSED (no sales redirect found)")
        ...
        for phrase in not_found_indicators:
            if phrase in content:
                logger.warning(
                    f"🔄 Fallback TRIGGERED: Found 'not found' indicator: '{phrase}'")
                return True
        ...
        if "[1]" in content or "**sources:**" in content:
            if not pinecone_response.get("has_citations", False):
                logger.warning(
                    "🚨 SAFETY OVERRIDE: Detected fake citations. Switching to web search.")
                return True
        logger.warning("✅ Citation safety check PASSED")

        # RULE 4: Force web search for regulatory topics without citations
        regulatory_indicators = [
            "regulation", "directive", "compliance", "legal"]
        is_regulatory = any(
            indicator in original_lower for indicator in regulatory_indicators)

        if is_regulatory and not pinecone_response.get("has_citations", False):
            logger.warning(
                "🔄 Fallback TRIGGERED: Regulatory topic without citations.")
            return True
        logger.warning(
            f"✅ Regulatory check PASSED (is_regulatory={is_regulatory})")
        ...
        logger.warning(
            "✅ ALL CHECKS PASSED - Should NOT fallback, but something is wrong!")
        logger.warning(
            "✅ Fallback SKIPPED: Pinecone provided a valid response from its knowledge base.")
        return False

    # CHANGE 6: Business-First Routing (Always Try Pinecone First) (from B)
    ...
    def get_response(self, prompt: str, chat_history: List[Dict] = None) -> Dict[str, Any]:
        ...
        if self.pinecone_tool:
            try:
                ...
                if pinecone_response:
                    ...
                    logger.warning(
                        f"   Success: {pinecone_response.get('success', False)}")
                    logger.warning(
                        f"   Content length: {len(pinecone_response.get('content', ''))}")
                    logger.warning(
                        f"   Has citations: {pinecone_response.get('has_citations', False)}")
                else:
                ...
                if pinecone_response and pinecone_response.get("success"):
                    ...
                    if not self.should_use_web_fallback(pinecone_response, prompt):
                        logger.info(
                            "✅ Using Pinecone response (passed business logic checks).")
                        error_handler.mark_component_healthy("Pinecone")
                        return pinecone_response
                    else:
                        logger.warning(
                            "⤵️ Pinecone response requires web fallback. Proceeding to Tavily...")

            except Exception as e:
                error_type = self._detect_pinecone_error_type(e)
                logger.error(
                    f"Pinecone query failed ({error_type}): {e}. Proceeding to web fallback.")
        ...
        if self.tavily_agent:
            try:
                ...
                logger.error(
                    f"FiFi Web search failed: {e}", exc_info=True)
                error_handler.log_error(
                    error_handler.handle_api_error("Tavily", "Query", e))

        # --- Final Fallback: If all systems fail ---
        logger.warning(
            "⚠️ All AI tools failed. Using final system fallback response.")
        return {
...
def check_content_moderation(prompt: str, client: Optional[openai.OpenAI]) -> Optional[Dict[str, Any]]:  # From B
    ...
    if not client or not hasattr(client, 'moderations'):
        logger.debug(
            "OpenAI client or moderation API not available. Skipping content moderation.")
        return {"flagged": False}

    try:
        response = client.moderations.create(
            model="omni-moderation-latest", input=prompt)
        ...
        if result.flagged:
            ...
            logger.warning(
                f"Input flagged by moderation for: {', '.join(flagged_categories)}")
            return {
                "flagged": True,
...
# CHANGE 1: Enhanced Validation Function with Context (from B)
...
def check_industry_context(prompt: str, chat_history: List[Dict] = None, client: Optional[openai.OpenAI] = None) -> Optional[Dict[str, Any]]:
    ...
    if not client or not hasattr(client, 'chat'):
        logger.debug(
            "OpenAI client not available for industry context check. Allowing question.")
...
{{
    ...
    "reason": "Brief explanation."
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {"role": "system",
                    "content": "You are an industry context validator. Respond only with valid JSON."},
        ...
        if not all(key in result for key in ['relevant', 'confidence', 'category', 'reason']):
            logger.warning(
                "Industry context check returned incomplete JSON structure")
            return {"relevant": True, "reason": "context_check_invalid_response"}

        logger.info(
            f"Context check: relevant={result['relevant']}, category={result['category']}, confidence={result['confidence']:.2f} (Context HELPED validate follow-up: {'yes' if result['category'] == 'follow_up' else 'no'})")
        return result
...
# =============================================================================
# SESSION MANAGER - MAIN ORCHESTRATOR CLASS (Merged from A, B)
# =============================================================================
...
class SessionManager:
    ...
    def get_session_timeout_minutes(self) -> int:  # From A
        """Returns the configured session timeout duration in minutes."""
        return 5

    # Helper to get privilege level for user types (from A)
    ...
    def _periodic_cleanup(self):  # From A
        ...
        try:
            ...
            if hasattr(self.fingerprinting, 'fingerprint_cache'):
                ...
                if old_entries:
                    logger.info(
                        f"Cleaned up {len(old_entries)} old fingerprint cache entries")
            ...
            if hasattr(self.rate_limiter, 'requests'):
                ...
                for identifier, timestamps in self.rate_limiter.requests.items():
                    ...
                    self.rate_limiter.requests[identifier] = [
                ...
                if old_limit_entries:
                    logger.info(
                        f"Cleaned up {len(old_limit_entries)} old rate limiter entries")
    ...
    def _update_activity(self, session: UserSession):  # From A
        ...
        if session.timeout_saved_to_crm:
            ...
            logger.info(
                f"Reset 'timeout_saved_to_crm' flag for session {session.session_id[:8]} due to new activity.")
        ...
        if not isinstance(session.messages, list):
            logger.warning(
                f"Messages field corrupted for session {session.session_id[:8]}, resetting to empty list")
            session.messages = []

        try:
            self.db.save_session(session)
            logger.debug(
                f"Activity update saved for {session.session_id[:8]} with {len(session.messages)} messages")
        except Exception as e:
            logger.error(
                f"Failed to save session during activity update: {e}", exc_info=True)

    def _save_session_with_retry(self, session: UserSession, max_retries: int = 3):  # From A
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    logger.error(
                        f"All session save attempts failed for {session.session_id[:8]}")
                    raise

    def _clear_error_notifications(self):  # From B (added 'pricing_stock_notice')
        """Clear all error notification states when a successful question is processed."""
        error_keys = ['rate_limit_hit', 'moderation_flagged',
                      'context_flagged', 'pricing_stock_notice']  # Added 'pricing_stock_notice'
        cleared_errors = []
        ...
        if cleared_errors:
            logger.debug(
                f"Auto-cleared error notifications on successful question: {cleared_errors}")

    def _create_new_session(self) -> UserSession:  # From A
        ...
        logger.debug(
            f"📍 Called from: {caller.filename}:{caller.lineno} in function {caller.function}")
        ...
        logger.debug(
            f"🆔 New session created: {session_id[:8]} (NOT saved to DB yet, will be saved in get_session)")
        return session

    def _check_15min_eligibility(self, session: UserSession) -> bool:  # From A
        ...
        try:
            ...
            logger.info(
                f"15-min eligibility check for {session.session_id[:8]}: {elapsed_minutes:.1f} minutes elapsed")
            return elapsed_minutes >= 15.0

        except Exception as e:
            logger.error(
                f"Error checking 15-min eligibility for {session.session_id[:8]}: {e}")
            return False

    def _is_crm_save_eligible(self, session: UserSession, trigger_reason: str) -> bool:  # From A
        ...
        try:
            if not session.email or not session.messages:
                logger.debug(
                    f"CRM save not eligible - missing email ({bool(session.email)}) or messages ({bool(session.messages)}) for {session.session_id[:8]}")
                return False

            if session.timeout_saved_to_crm and "clear_chat" in trigger_reason.lower():
                logger.debug(
                    f"CRM save not eligible - already saved for {session.session_id[:8]}")
                return False
            ...
            if session.user_type.value not in [UserType.REGISTERED_USER.value, UserType.EMAIL_VERIFIED_GUEST.value]:  # FIXED: Corrected to .value for enum comparison
                logger.debug(
                    f"CRM save not eligible - user type {session.user_type.value} for {session.session_id[:8]}")
                return False

            if session.daily_question_count < 1:
                logger.debug(
                    f"CRM save not eligible - no questions asked ({session.daily_question_count}) for {session.session_id[:8]}")
                return False

            logger.info(
                f"CRM save eligible for {session.session_id[:8]}: UserType={session.user_type.value}, Questions={session.daily_question_count}")
            return True

        except Exception as e:
            logger.error(
                f"Error checking CRM eligibility for {session.session_id[:8]}: {e}")
            return False

    def _is_manual_crm_save_eligible(self, session: UserSession) -> bool:  # From A
        ...
        try:
            ...
            logger.info(
                f"Manual CRM save eligible for {session.session_id[:8]}: UserType={session.user_type.value}, Questions={session.daily_question_count}")
            return True

        except Exception as e:
            logger.error(
                f"Error checking manual CRM eligibility for {session.session_id[:8]}: {e}")
            return False

    def _attempt_fingerprint_inheritance(self, session: UserSession):  # From A
        ...
        """
        logger.info(
            f"🔄 Attempting fingerprint inheritance for session {session.session_id[:8]} with fingerprint {session.fingerprint_id[:8]}...")

        try:
            # 1. Get all sessions associated with this fingerprint, including the current one
            historical_fp_sessions = self.db.find_sessions_by_fingerprint(
                session.fingerprint_id)

            # Filter out the current session from historical list, if it was already saved
            historical_fp_sessions = [
                s for s in historical_fp_sessions if s.session_id != session.session_id]
            ...
            # Use current session's LQT as baseline
            ...
            # FIX: Initialize this variable
            merged_email_switches_count = session.email_switches_count
            ...
            for s in all_sessions_for_merge:
                ...
                if s.daily_question_count is not None:
                    ...
                    if s.active or (s.last_question_time and (now - s.last_question_time < timedelta(hours=24))):
                        merged_daily_question_count = max(
                            merged_daily_question_count, s.daily_question_count)
                    else:
                ...
                if s.total_question_count is not None:
                    merged_total_question_count = max(
                        merged_total_question_count, s.total_question_count)
                ...
                if s.evasion_count is not None:
                    merged_evasion_count = max(
                        merged_evasion_count, s.evasion_count)
                if s.current_penalty_hours is not None:
                    merged_current_penalty_hours = max(
                        merged_current_penalty_hours, s.current_penalty_hours)
                if s.escalation_level is not None:
                    merged_escalation_level = max(
                        merged_escalation_level, s.escalation_level)

                # Merge email addresses used
                if s.email_addresses_used:
                    merged_email_addresses_used.update(s.email_addresses_used)
                if s.email_switches_count is not None:
                    merged_email_switches_count = max(
                        merged_email_switches_count, s.email_switches_count)
            ...
            # Set from the most recent activity found
            ...
            if self._get_privilege_level(source_for_identity_and_base_data.user_type) > self._get_privilege_level(session.user_type):
                ...
                logger.info(
                    f"Re-verification pending for {session.session_id[:8]}. Higher privilege ({source_for_identity_and_base_data.user_type.value}) found for fingerprint, but requires email verification.")
            else:
                ...
                # This ensures GUEST remains GUEST if no higher is found
                ...
                session.browser_privacy_level = source_for_identity_and_base_data.browser_privacy_level
                # No pending if already at max privilege or no higher exists
                session.reverification_pending = False
                session.pending_user_type = None
                session.pending_email = None
                session.pending_full_name = None
                ...
                logger.info(
                    f"No re-verification needed for {session.session_id[:8]}. User type set to {session.user_type.value}.")
            ...
            if session.fingerprint_id.startswith(("temp_py_", "temp_fp_", "fallback_")) and \
               not source_for_identity_and_base_data.fingerprint_id.startswith(("temp_py_", "temp_fp_", "fallback_")):
                ...
                logger.info(
                    f"Updating temporary fingerprint {session.fingerprint_id[:8]} to recognized fingerprint {source_for_identity_and_base_data.fingerprint_id[:8]}")
            ...
            logger.info(
                f"Inheritance complete for {session.session_id[:8]}: user_type={session.user_type.value} (from {original_session_user_type_for_log.value}), daily_q={session.daily_question_count}, total_q={session.total_question_count}, fp={session.fingerprint_id[:8]}, active={session.active}, rev_pending={session.reverification_pending}")

        except Exception as e:
            logger.error(
                f"Error during fingerprint inheritance for session {session.session_id[:8]}: {e}", exc_info=True)

    def get_session(self) -> Optional[UserSession]:  # From A
        """Gets or creates the current user session with enhanced validation."""
        logger.info(
            f"🔍 get_session() called - current_session_id in state: {st.session_state.get('current_session_id', 'None')}")
        ...
        try:
            ...
            if session_id:
                ...
                if st.session_state.get(f'loading_{session_id}', False):
                    logger.warning(
                        f"Session {session_id[:8]} already being loaded, skipping")
                    return None
                ...
                if session and session.active:
                    ...
                    if (session.fingerprint_id and
                        ...
                        logger.info(
                            f"Fingerprint inheritance check and save completed for {session.session_id[:8]}")

                    # Check limits and handle bans. This is where the 24-hour reset happens.
                    limit_check = self.question_limits.is_within_limits(
                        session)
                    if not limit_check.get('allowed', True):
                        ...
                        # This condition seems redundant based on the `if not allowed and reason != guest_limit` above
                        if session.reverification_pending and limit_check.get('reason') == 'guest_limit':
                            logger.info(
                                f"Session {session.session_id[:8]} is pending re-verification, suppressing ban message and allowing dialog.")
                        else:
                            ban_type = limit_check.get('ban_type', 'unknown')
                            message = limit_check.get(
                                'message', 'Access restricted due to usage policy.')
                            ...
                            st.info(message)
                            logger.info(
                                f"Session {session_id[:8]} is currently banned: Type={ban_type}, Reason='{message}'.")

                        try:
                            self.db.save_session(session)
                        except Exception as e:
                            logger.error(
                                f"Failed to save banned session {session.session_id[:8]}: {e}", exc_info=True)
                    ...
                    logger.warning(
                        f"Session {session_id[:8]} not found or inactive. Creating new session.")
            ...
            # <--- This call will now also correctly set visitor_type
            ...
            # Save the new session (potentially updated by inheritance)
            self.db.save_session(new_session)
            logger.info(
                f"Created and stored new session {new_session.session_id[:8]} (post-inheritance check), active={new_session.active}, rev_pending={new_session.reverification_pending}")
            return new_session

        except Exception as e:
            logger.error(f"Failed to get/create session: {e}", exc_info=True)
            fallback_session = UserSession(session_id=str(
                uuid.uuid4()), user_type=UserType.GUEST, last_activity=None)
            ...
            st.error(
                "⚠️ Failed to create or load session. Operating in emergency fallback mode. Chat history may not persist.")
            logger.error(
                f"Emergency fallback session created {fallback_session.session_id[:8]}")
            return fallback_session

    def apply_fingerprinting(self, session: UserSession, fingerprint_data: Dict[str, Any]) -> bool:  # From A
        """Applies fingerprinting data from custom component to the session with better validation."""
        logger.debug(
            f"🔍 APPLYING FINGERPRINT received from JS: {fingerprint_data.get('fingerprint_id', 'None')[:8]} to session {session.session_id[:8]}")

        try:
            if not fingerprint_data or not isinstance(fingerprint_data, dict):
                logger.warning(
                    "Invalid fingerprint data provided to apply_fingerprinting")
                return False
            ...
            session.fingerprint_method = fingerprint_data.get(
                'fingerprint_method')
            # session.visitor_type is determined by apply_fingerprinting after inheritance check
            session.browser_privacy_level = fingerprint_data.get(
                'browser_privacy_level', 'standard')
            ...
            if not session.fingerprint_id or not session.fingerprint_method:
                logger.error(
                    "Invalid fingerprint data: missing essential fields from JS. Reverting to old fingerprint.")
            ...
            # <--- This call will now also update visitor_type
            self._attempt_fingerprint_inheritance(session)
            ...
            try:
                self.db.save_session(session)
                logger.info(
                    f"✅ Fingerprinting applied and inheritance checked for {session.session_id[:8]}: {session.fingerprint_method} (ID: {session.fingerprint_id[:8]}...), active={session.active}, rev_pending={session.reverification_pending}")
            except Exception as e:
                logger.error(
                    f"Failed to save session after fingerprinting (JS data received): {e}")
    ...
    def check_fingerprint_history(self, fingerprint_id: str) -> Dict[str, Any]:  # From A
        """Check if a fingerprint has historical sessions and return relevant information."""
        try:
            existing_sessions = self.db.find_sessions_by_fingerprint(
                fingerprint_id)
            ...
            if most_privileged_session:
                return {
                    ...
                    # Also pass the count from history
    ...
    def handle_guest_email_verification(self, session: UserSession, email: str) -> Dict[str, Any]:  # From A
        ...
        try:
            ...
            if self.question_limits.detect_guest_email_evasion(session, self.db):
                penalty_hours = self.question_limits.apply_evasion_penalty(
                    session)
                self.db.save_session(session)  # Save the penalty to database
                logger.warning(
                    f"🚨 EMAIL EVASION DETECTED: Applied {penalty_hours}h penalty to session {session.session_id[:8]}")
                return {
                    'success': False,
            ...
            if not session.reverification_pending:
                ...
                session.pending_wp_token = None
                logger.warning(
                    f"Session {session.session_id[:8]} switched email during pending re-verification. Resetting pending state.")
            ...
            try:
                ...
                logger.error(
                    f"Failed to save session before email verification: {e}")

            # Send verification code
            code_sent = self.email_verification.send_verification_code(
                sanitized_email)
    ...
    def verify_email_code(self, session: UserSession, code: str) -> Dict[str, Any]:  # From A
        ...
        try:
            ...
            verification_success = self.email_verification.verify_code(
                email_to_verify, sanitized_code)

            if verification_success:
                logger.info(
                    f"✅ BEFORE email verification upgrade for session {session.session_id[:8]}:")
                logger.info(f"   - User Type: {session.user_type.value}")
                logger.info(
                    f"   - Daily Questions: {session.daily_question_count}")
                logger.info(
                    f"   - Total Questions: {session.total_question_count}")
                logger.info(f"   - Has Messages: {len(session.messages)}")
                logger.info(
                    f"   - Reverification Pending: {session.reverification_pending}")
                ...
                if session.reverification_pending:
                    ...
                    session.pending_wp_token = None
                    logger.info(
                        f"✅ User {session.session_id[:8]} reclaimed higher privilege: {session.user_type.value} via re-verification for {session.email}")
                else:
                    ...
                    logger.info(
                        f"✅ User {session.session_id[:8]} upgraded to EMAIL_VERIFIED_GUEST (first time): {session.email}")
                ...
                try:
                    self.db.save_session(session)
                    logger.info(
                        f"✅ AFTER email verification upgrade for session {session.session_id[:8]}:")
                    logger.info(f"   - User Type: {session.user_type.value}")
                    logger.info(
                        f"   - Daily Questions: {session.daily_question_count} (PRESERVED from device history)")
                    logger.info(
                        f"   - Total Questions: {session.total_question_count}")
                    logger.info(
                        f"   - Has Messages: {len(session.messages)}")
                    logger.info(
                        f"   - CRM Eligible: {self._is_crm_save_eligible(session, 'email_verification_success')}")
    ...
    def authenticate_with_wordpress(self, username: str, password: str) -> Optional[UserSession]:  # From A
        """Authenticates user with WordPress and creates/updates session."""
        if not self.config.WORDPRESS_URL:
            st.error(
                "WordPress authentication is not configured. Please contact support.")
            logger.warning(
                "WordPress authentication attempted but URL not configured.")
            return None
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if 'application/json' not in content_type:
                    logger.error(
                        f"WordPress authentication received unexpected Content-Type '{content_type}' with 200 status. Expected application/json. Response text: '{response.text[:200]}'", exc_info=True)
                    st.error(
                        "Authentication failed: WordPress returned a webpage instead of an API response. This often means the JWT authentication plugin is not active or configured correctly on the WordPress site. Please contact support.")
                    return None
                ...
                try:
                    ...
                    logger.error(
                        f"WordPress authentication received non-JSON response with 200 status, despite Content-Type possibly being JSON. Response text: '{response.text[:200]}'. Error: {e}", exc_info=True)
                    st.error(
                        "Authentication failed: Invalid JSON response from WordPress. Please try again or contact support.")
                    return None
                ...
                if wp_token and user_email:
                    ...
                    if current_session.reverification_pending:
                        logger.info(
                            f"REGISTERED_USER login for {current_session.session_id[:8]} supersedes pending re-verification.")
                    ...
                    try:
                        ...
                        logger.info(
                            f"User authenticated and upgraded to REGISTERED_USER: {user_email}, daily_q={current_session.daily_question_count}")
                    except Exception as e:
                        logger.error(
                            f"Failed to save authenticated session: {e}")
                        st.error(
                            "Authentication succeeded but session could not be saved. Please try again or contact support.")
                        return None

                    return current_session
                else:
                    logger.error(
                        f"WordPress authentication successful (status 200) but missing token or email in response. Response: {data}")
                    st.error(
                        "Authentication failed: Incomplete response from WordPress. Please contact support.")
                    return None
            else:
                logger.warning(
                    f"WordPress authentication failed with status: {response.status_code}. Response: {response.text[:200]}")
            ...
            st.error(
                "Could not connect to authentication service. Check your internet connection or try again later.")
            return None
        except Exception as e:
            logger.error(
                f"An unexpected error occurred during WordPress authentication: {e}", exc_info=True)
            st.error(
                "An unexpected error occurred during authentication. Please try again later.")
            return None

    # CHANGE 10: Pricing/Stock Question Handler (from B)
    def detect_pricing_stock_question(self, prompt: str) -> bool:
        ...
        has_pricing = any(
            indicator in prompt_lower for indicator in pricing_indicators)
        has_stock = any(
            indicator in prompt_lower for indicator in stock_indicators)

        return has_pricing or has_stock

    # CHANGE 11: Solution 1 - Meta-Question Detection (Token-Free) (from B)
    ...
    def handle_meta_conversation_query(self, session: UserSession, query_type: str, scope: str) -> Dict[str, Any]:  # From B
        ...
        user_questions = [
            msg['content'] for msg in visible_messages if msg.get('role') == 'user']
...
• **Daily Usage**: {session.daily_question_count} questions today""",
            ...
            questions_to_show = user_questions[-20:] if len(
                user_questions) > 20 else user_questions
            start_number = len(user_questions) - \
                len(questions_to_show) + 1 if len(user_questions) > 20 else 1
    ...
    def _generate_conversation_summary(self, user_questions: List[str], session: UserSession) -> Dict[str, Any]:  # From B
        ...
        for question in user_questions:
            ...
            words = [w for w in q_lower.split() if len(w) > 4 and w not in [
                'what', 'where', 'when', 'which', 'about', 'would', 'could', 'should']]
        ...
        if active_categories:
            ...
            for category, count in active_categories.items():
                summary_parts.append(
                    f"• **{category.title()}**: {count} question{'s' if count > 1 else ''}")
    ...
    def _analyze_conversation_topics(self, user_questions: List[str], session: UserSession) -> Dict[str, Any]:  # From B
        ...
        ingredient_indicators = ['extract', 'powder',
                                 'oil', 'acid', 'syrup', 'sweetener', 'flavor', 'color']
        ...
        if ingredients_mentioned:
            analysis_parts.append(
                f"• **Ingredients Discussed**: {', '.join(list(ingredients_mentioned)[:6])}")

        if business_aspects:
            analysis_parts.append(
                f"• **Business Aspects**: {', '.join(list(business_aspects))}")

        if technical_terms:
            analysis_parts.append(
                f"• **Technical Focus**: {', '.join(list(technical_terms))}")
        ...
        if focus_areas:
            analysis_parts.append(
                f"• **Industry Focus**: {', '.join(focus_areas)}")
    ...
    def get_ai_response(self, session: UserSession, prompt: str) -> Dict[str, Any]:  # Merged from A and B
        ...
        try:
            ...
            if rate_limiter_id is None or rate_limiter_id.startswith(("temp_py_", "temp_fp_", "fallback_")):
                ...
                logger.warning(
                    f"Rate limiter using session ID as fallback for unconfirmed fingerprint: {rate_limiter_id[:8]}...")
            ...
            moderation_result = check_content_moderation(
                prompt, self.ai.openai_client)
            if moderation_result and moderation_result.get("flagged"):
                categories = moderation_result.get('categories', [])
                logger.warning(
                    f"Content moderation flagged input: {categories}")
            ...
            # NEW: Check if it's a meta-conversation query (Solution 1) (From B)
            meta_detection = self.detect_meta_conversation_query(prompt)
            if meta_detection["is_meta"]:
                logger.info(
                    f"Meta-conversation query detected: {meta_detection['type']}")
            ...
            # CHANGE 2: Update the function call to pass conversation history (From B)
            context_result = check_industry_context(
                prompt, session.messages, self.ai.openai_client)
            if context_result and not context_result.get("relevant", True):
                ...
                reason = context_result.get(
                    "reason", "Not relevant to food & beverage ingredients industry")

                logger.warning(
                    f"Industry context check flagged input: category={category}, confidence={confidence:.2f}, reason={reason}")
            ...
            # NEW: Check for pricing/stock questions (from B)
...
Please proceed with your question, keeping in mind that any pricing or stock information provided should be verified through official channels."""
            ...
            if not limit_check['allowed']:
                ...
                if limit_check.get('reason') == 'registered_user_tier1_limit':
                    self.question_limits._apply_ban(
                        session, BanStatus.ONE_HOUR, "Tier 1 limit reached (10 questions)")
                    # Important: Update activity and save immediately after applying ban
                    self._update_activity(session)
                    logger.info(
                        f"Registered User Tier 1 (10 questions) ban applied for session {session.session_id[:8]} at question attempt.")
                elif limit_check.get('reason') == 'registered_user_tier2_limit':
                    self.question_limits._apply_ban(
                        session, BanStatus.TWENTY_FOUR_HOUR, "Registered user daily limit reached (20 questions)")
                    self._update_activity(session)
                    logger.info(
                        f"Registered User Tier 2 (20 questions) ban applied for session {session.session_id[:8]} at question attempt.")
                elif limit_check.get('reason') == 'email_verified_guest_limit':
                    self.question_limits._apply_ban(
                        session, BanStatus.TWENTY_FOUR_HOUR, "Email-verified daily limit reached (10 questions)")
                    self._update_activity(session)
                    logger.info(
                        f"Email Verified Guest (10 questions) ban applied for session {session.session_id[:8]} at question attempt.")
                ...
                return {
                    'banned': True,
                    'content': ban_message,
                    # This will be populated from is_within_limits
                    'time_remaining': limit_check.get('time_remaining')
                }
            ...
            self.question_limits.record_question(session)
            logger.info(
                f"Question recorded for session {session.session_id[:8]} AFTER all pre-checks.")

            # Get AI response
            ai_response = self.ai.get_response(
                sanitized_prompt, session.messages)
            ...
            logger.error(
                f"AI response generation failed: {e}", exc_info=True)
            return {
    ...
    def clear_chat_history(self, session: UserSession):  # From A
        ...
        try:
            ...
            logger.info(
                f"Soft cleared chat for session {session.session_id[:8]}: offset set to {session.display_message_offset}")

        except Exception as e:
            logger.error(
                f"Failed to clear chat history for session {session.session_id[:8]}: {e}")

    def manual_save_to_crm(self, session: UserSession):  # From A
        ...
        try:
            if self._is_manual_crm_save_eligible(session):
                with st.spinner("Saving chat transcript to CRM..."):
                    success = self.zoho.save_chat_transcript_sync(
                        session, "Manual Save")

                if success:
                    st.success("✅ Chat transcript saved to CRM successfully!")
                    logger.info(
                        f"Manual CRM save successful for session {session.session_id[:8]}")
                else:
                    st.error("❌ Failed to save to CRM. Please try again later.")
                    logger.warning(
                        f"Manual CRM save failed for session {session.session_id[:8]}")
            else:
                st.warning(
                    "⚠️ CRM save not available. Ensure you have an email verified and at least 1 question asked.")
                logger.info(
                    f"Manual CRM save not eligible for session {session.session_id[:8]}")

        except Exception as e:
            logger.error(f"Manual CRM save error: {e}")
            st.error("❌ An error occurred while saving to CRM.")

    def end_session(self, session: UserSession):  # From A
        ...
        try:
                ...
                with st.spinner("Saving your conversation..."):
                    try:
                        ...
                        if success:
                            ...
                            st.warning(
                                "⚠️ Conversation save failed, but sign out will continue.")
                    except Exception as e:
                        logger.error(
                            f"CRM save during sign out failed: {e}")
                        st.warning(
                            "⚠️ Conversation save failed, but sign out will continue.")
            ...
            try:
                ...
                logger.error(
                    f"Failed to save session during end_session: {e}")
...
def render_simple_activity_tracker(session_id: str):  # From A
    """Renders a simple activity tracker that monitors user interactions."""
    if not session_id:
        logger.warning(
            "render_simple_activity_tracker called without session_id")
        return None
    ...
    try:
        ...
        logger.error(
            f"Problematic key in render_simple_activity_tracker: {component_key}")
        return None


def check_timeout_and_trigger_reload(session_manager: 'SessionManager', session: UserSession, activity_result: Optional[Dict[str, Any]]) -> bool:  # From A
    ...
    if fresh_session_from_db:
        ...
        # Corrected variable name from fresh_session_from_rb
        ...
        logger.warning(
            f"Session {session.session_id[:8]} from st.session_state not found in database. Forcing reset.")
        session.active = False

    # If the session is already inactive, force a reload
    if not session.active:
        logger.info(
            f"Session {session.session_id[:8]} is inactive. Triggering reload to welcome page.")
        ...
        if JS_EVAL_AVAILABLE:
            try:
                ...
                logger.error(
                    f"Browser reload failed during inactive session handling: {e}")
    ...
    if session.last_activity is None:
        logger.debug(
            f"Session {session.session_id[:8]}: last_activity is None, timer has not started.")
        return False
    ...
    if activity_result:
        try:
            ...
            if js_last_activity_timestamp:
                new_activity_dt = datetime.fromtimestamp(
                    js_last_activity_timestamp / 1000)

                if new_activity_dt > session.last_activity:
                    logger.debug(
                        f"Updating last_activity for {session.session_id[:8]} from JS: {session.last_activity.strftime('%H:%M:%S')} -> {new_activity_dt.strftime('%H:%M:%S')}")
                    session.last_activity = new_activity_dt
                    session_manager._save_session_with_retry(session)
        except Exception as e:
            logger.error(
                f"Error processing client JS activity timestamp for session {session.session_id[:8]}: {e}")
    ...
    logger.info(
        f"TIMEOUT CHECK: Session {session.session_id[:8]} | Inactive: {minutes_inactive:.1f}m | last_activity: {session.last_activity.strftime('%H:%M:%S')}")

    # Check if timeout duration has passed
    if minutes_inactive >= session_manager.get_session_timeout_minutes():
        logger.info(
            f"⏰ TIMEOUT DETECTED: {session.session_id[:8]} inactive for {minutes_inactive:.1f} minutes")

        # Perform CRM save if eligible
        if session_manager._is_crm_save_eligible(session, "timeout_auto_reload"):
            logger.info(
                f"💾 Performing emergency save before auto-reload for {session.session_id[:8]}")
            try:
                emergency_data = {
                ...
                response = requests.post(
                    fastapi_url, json=emergency_data, timeout=5)
                if response.status_code == 200:
                    logger.info(
                        f"✅ Emergency save sent to FastAPI successfully")
                else:
                    logger.warning(
                        f"⚠️ FastAPI returned status {response.status_code}, using local fallback")
                    session_manager.zoho.save_chat_transcript_sync(
                        session, "timeout_auto_reload_fallback")
                    session.timeout_saved_to_crm = True
            except Exception as e:
                logger.error(f"❌ Failed to send emergency save to FastAPI: {e}")
                try:
                    logger.info(
                        f"🔄 Using local CRM save as fallback for timeout")
                    session_manager.zoho.save_chat_transcript_sync(
                        session, "timeout_auto_reload_fallback")
                    session.timeout_saved_to_crm = True
...
def render_simplified_browser_close_detection(session_id: str):  # From A
    ...
    if not session:
        logger.debug(
            f"No session found for browser close detection: {session_id[:8]}")
        return
    ...
    if not session_manager._is_crm_save_eligible(session, "browser_close_check"):
        logger.info(
            f"🚫 Session {session_id[:8]} not eligible for CRM save - skipping browser close detection")
        return

    # Only set up emergency save for eligible users
    logger.info(
        f"✅ Setting up browser close detection for eligible session {session_id[:8]}")
...
def process_fingerprint_from_query(session_id: str, fingerprint_id: str, method: str, privacy: str, working_methods: List[str]) -> bool:  # From A
    """Processes fingerprint data received via URL query parameters."""
    try:
        session_manager = st.session_state.get('session_manager')
        if not session_manager:
            logger.error(
                "❌ Session manager not available during fingerprint processing from query.")
            return False

        session = session_manager.db.load_session(session_id)
        if not session:
            logger.error(
                f"❌ Fingerprint processing: Session '{session_id[:8]}' not found in database.")
            return False

        logger.info(
            f"✅ Processing fingerprint for session '{session_id[:8]}': ID={fingerprint_id[:8]}, Method={method}, Privacy={privacy}")
        ...
        if success:
            logger.info(
                f"✅ Fingerprint applied successfully to session '{session_id[:8]}'")
            st.session_state.is_chat_ready = True  # NEW: Explicitly unlock chat input here
            st.session_state.fingerprint_status = 'done'  # Mark fingerprinting as done (from B)
            logger.info(
                f"Chat input unlocked for session {session_id[:8]} after successful JS fingerprinting.")
            return True
        else:
            logger.warning(
                f"⚠️ Fingerprint application failed for session '{session_id[:8]}'")
            return False
...
def process_emergency_save_from_query(session_id: str, reason: str) -> bool:  # From A
    """Processes emergency save request from query parameters."""
    try:
        session_manager = st.session_state.get('session_manager')
        if not session_manager:
            logger.error(
                "❌ Session manager not available during emergency save processing.")
            return False

        session = session_manager.db.load_session(session_id)
        if not session:
            logger.error(
                f"❌ Emergency save: Session '{session_id[:8]}' not found in database.")
            return False

        logger.info(
            f"🚨 Processing emergency save for session '{session_id[:8]}', reason: {reason}")
        ...
        if session_manager._is_crm_save_eligible(session, f"Emergency Save: {reason}"):
            success = session_manager.zoho.save_chat_transcript_sync(
                session, f"Emergency Save: {reason}")
            if success:
                session.timeout_saved_to_crm = True
                ...
                session_manager.db.save_session(session)
                logger.info(
                    f"✅ Emergency save completed successfully for session {session_id[:8]}")
                return True
            else:
                logger.warning(
                    f"⚠️ Emergency save CRM operation failed for session {session_id[:8]}")
                return False
        else:
            logger.info(
                f"ℹ️ Emergency save not eligible for CRM save for session {session_id[:8]}")
            return False
...
def handle_emergency_save_requests_from_query():  # From A
    """Checks for and processes emergency save requests sent via URL query parameters."""
    logger.info(
        "🔍 EMERGENCY SAVE HANDLER: Checking for query parameter requests for emergency save...")
    ...
    if event == "emergency_close" and session_id:
        logger.info("=" * 80)
        logger.info(
            "🚨 EMERGENCY SAVE REQUEST DETECTED VIA URL QUERY PARAMETERS!")
        logger.info(f"Session ID: {session_id}, Event: {event}, Reason: {reason}")
        if fallback == "true":
            logger.warning(
                "⚠️ THIS IS A FALLBACK SAVE - FastAPI beacon likely failed!")
        logger.info("=" * 80)

        st.error(
            "🚨 **Emergency Save Detected** - Processing browser close save...")
        if fallback == "true":
        ...
        try:
            ...
            success = process_emergency_save_from_query(
                session_id, save_reason)

            if success:
                st.success("✅ Emergency save completed successfully!")
                logger.info(
                    "✅ Emergency save completed via query parameter successfully.")
            else:
                st.info(
                    "ℹ️ Emergency save completed (no CRM save needed or failed).")
                logger.info(
                    "ℹ️ Emergency save completed via query parameter (not eligible for CRM save or internal error).")

        except Exception as e:
            st.error(
                f"❌ An unexpected error occurred during emergency save: {str(e)}")
            logger.critical(
                f"Emergency save processing crashed from query parameter: {e}", exc_info=True)

        # Removed time.sleep(2)
        st.stop()
    else:
        logger.debug(
            "ℹ️ No emergency save requests found in current URL query parameters.")


def handle_fingerprint_requests_from_query():  # From A
    """Checks for and processes fingerprint data sent via URL query parameters."""
    logger.info(
        "🔍 FINGERPRINT HANDLER: Checking for query parameter fingerprint data...")
    ...
    if event == "fingerprint_complete" and session_id:
        ...
        working_methods = query_params.get("working_methods", "").split(
            ",") if query_params.get("working_methods") else []

        logger.info(
            f"Extracted - ID: {fingerprint_id}, Method: {method}, Privacy: {privacy}, Working Methods: {working_methods}")

        # Clear query parameters AFTER extraction
        params_to_clear = ["event", "session_id", "fingerprint_id",
                           "method", "privacy", "working_methods", "timestamp"]
        ...
        if not fingerprint_id or not method:
            st.error(
                "❌ **Fingerprint Error** - Missing required data in redirect")
            logger.error(
                f"Missing fingerprint data: ID={fingerprint_id}, Method={method}")
            # Removed time.sleep(2)
            st.rerun()
            return

        try:
            success = process_fingerprint_from_query(
                session_id, fingerprint_id, method, privacy, working_methods)
            logger.info(f"✅ Silent fingerprint processing: {success}")

            if success:
                logger.info(
                    f"🔄 Fingerprint processed successfully, stopping execution to preserve page state")
                st.stop()
        except Exception as e:
        ...
        logger.debug(
            "ℹ️ No fingerprint requests found in current URL query parameters.")

# =============================================================================
# UI COMPONENTS
# =============================================================================

# Modified render_welcome_page function (from A)
...
def render_welcome_page(session_manager: 'SessionManager'):
    ...
    with tab1:
        if not session_manager.config.WORDPRESS_URL:
            st.warning(
                "Sign-in is currently disabled because the authentication service (WordPress URL) is not configured in application secrets.")
        else:
            with st.form("login_form", clear_on_submit=True):
                st.markdown("### 🔐 Sign In to Your Account")
                username = st.text_input(
                    "Username or Email", help="Enter your WordPress username or email.")
                password = st.text_input(
                    "Password", type="password", help="Enter your WordPress password.")
                ...
                if submit_button:
                    if not username or not password:
                        ...
                        set_loading_state(
                            True, "Authenticating and preparing your session...")
                        st.rerun()  # Immediately show loading state (NEW)

            st.markdown("---")
            st.info(
                "Don't have an account? [Register here](https://www.12taste.com/in/my-account/) to unlock full features!")
        ...
        **Continue as a guest** to get a quick start and try FiFi AI Assistant without signing in.
        ...
        with col2:
            if st.button("👤 Start as Guest", use_container_width=True):
                ...
                set_loading_state(
                    True, "Setting up your session and initializing AI assistant...")
                st.rerun()  # Immediately show loading state (NEW)
...
def render_sidebar(session_manager: 'SessionManager', session: UserSession, pdf_exporter: PDFExporter):  # From A
    ...
    with st.sidebar:
        ...
        if session.user_type.value == UserType.REGISTERED_USER.value:
            ...
            if session.daily_question_count <= 10:
                st.progress(min(session.daily_question_count / 10, 1.0),
                            text=f"Tier 1: {session.daily_question_count}/10 questions")
                remaining_tier1 = 10 - session.daily_question_count
                if remaining_tier1 > 0:
                    st.caption(
                        f"⏰ {remaining_tier1} questions until 1-hour break")
                else:
                    st.caption("🚫 Tier 1 complete - 1 hour break required")
            else:
                tier2_progress = min(
                    (session.daily_question_count - 10) / 10, 1.0)
                st.progress(tier2_progress,
                            text=f"Tier 2: {session.daily_question_count - 10}/10 questions")
            ...
            st.markdown(
                f"**Daily Questions:** {session.daily_question_count}/10")
            st.progress(min(session.daily_question_count / 10, 1.0))
            ...
            if session.reverification_pending:
                st.info(
                    "💡 An account is available for this device. Re-verify email to reclaim it!")
            # Check for this state
                ...
                st.info(
                    "💡 You are currently using guest questions. Verify your email to get more.")  # Alternative message
        ...
        if session.last_activity is not None:
            ...
            if minutes_inactive >= (timeout_duration - 1) and minutes_inactive < timeout_duration:
                ...
                st.error(
                    f"🚫 Session is likely expired. Type a question to check.")
        else:
        ...
        if ai_system:
            ...
            pinecone_status = error_handler.component_status.get(
                "Pinecone", "healthy")
            tavily_status = error_handler.component_status.get(
                "Tavily", "healthy")
            ...
            if ai_system.pinecone_tool and ai_system.pinecone_tool.assistant:
                if pinecone_status == "healthy":
                    ...
                    st.error(
                        f"🧠 Knowledge Base: {pinecone_status.replace('_', ' ').title()}")
            ...
            if ai_system.tavily_agent:
                if tavily_status == "healthy":
                    ...
                    st.warning(
                        f"🌐 Web Search: {tavily_status.replace('_', ' ').title()}")
        ...
        visible_messages = len(
            session.messages) - session.display_message_offset

        if session.display_message_offset > 0:
            st.markdown(
                f"**Messages in Chat:** {visible_messages} (Total: {total_messages})")
            st.caption(
                f"💡 {session.display_message_offset} messages hidden by Clear Chat")
        else:
        ...
        if session.ban_status.value != BanStatus.NONE.value:
            ...
            st.markdown(
                f"Reason: {session.ban_reason or 'Usage policy violation'}")
        ...
        with col1:
            ...
            if st.button("🗑️ Clear Chat", use_container_width=True, help=clear_chat_help):
                ...
                st.success(
                    "🗑️ Chat display cleared! Messages preserved in database.")
                st.rerun()
        ...
        if session.user_type.value in [UserType.REGISTERED_USER.value, UserType.EMAIL_VERIFIED_GUEST.value] and session.messages:
            ...
            if session_manager.zoho.config.ZOHO_ENABLED and session.email:
                ...
                st.caption(
                    "💡 Chat automatically saves to CRM during Sign Out or browser/tab close.")

# NEW FUNCTION FOR EMAIL PROMPT MANAGEMENT (From A)


def display_email_prompt_if_needed(session_manager: 'SessionManager', session: UserSession) -> bool:  # From A
    """
    ...
    # Use the explicitly evaluated component
    is_guest_limit_hit = (user_is_guest and daily_q_ge_limit)

    logger.debug(
        f"DEBUG: display_email_prompt_if_needed: session_id={session.session_id[:8]} | user_type={session.user_type.value} | daily_q={session.daily_question_count} | is_guest_limit_hit={is_guest_limit_hit}")
    ...
    if session.reverification_pending:
        ...
        if st.session_state.verification_stage is None:  # Initial entry to re-verification
            ...
            # Clear any previous guest-continue state
            st.session_state.guest_continue_active = False
        ...
        if st.session_state.verification_stage is None or st.session_state.verification_stage == 'declined_recognized_email_prompt_only':
            # Force email entry if limit hit
    ...
    if current_stage == 'initial_check':
        ...
        masked_email = session_manager._mask_email(
            email_to_reverify) if email_to_reverify else "your registered email"
        st.info(
            f"🤝 **We recognize this device was previously used as a {session.pending_user_type.value.replace('_', ' ').title()} account.**")
        st.info(
            f"Please verify **{masked_email}** to reclaim your status and higher question limits.")
        ...
        if email_to_verify:
            with st.spinner(f"Sending verification code to {session_manager._mask_email(email_to_verify)}..."):
                result = session_manager.handle_guest_email_verification(
                ...
                if result['success']:
                    ...
                    if "unusual activity" in result['message'].lower():
                        st.stop()
                    # Fallback to manual entry if send fails
                    st.session_state.verification_stage = "email_entry"
            st.rerun()

    elif current_stage == 'email_entry':
        st.info(
            "You've used your 4 free questions. Please verify your email to unlock 10 questions per day.")
        with st.form("email_verification_form", clear_on_submit=False):
            st.markdown(
                "**Please enter your email address to receive a verification code:**")
            current_email_input = st.text_input("Email Address", placeholder="your@email.com",
                                                value=st.session_state.get('verification_email', session.email or ""), key="manual_email_input")
        ...
        verification_email = st.session_state.get(
            'verification_email', session.email)
        st.success(
            f"📧 A verification code has been sent to **{session_manager._mask_email(verification_email)}**.")
        st.info(
            "Please check your email, including spam/junk folders. The code is valid for 1 minute. Need quick action!")

        with st.form("code_verification_form", clear_on_submit=False):
            code = st.text_input("Enter Verification Code",
                                 placeholder="e.g., 123456", max_chars=6, key="verification_code_input")
            col_code1, col_code2 = st.columns(2)
            with col_code1:
                submit_code = st.form_submit_button(
                    "Verify Code", use_container_width=True)
            with col_code2:
                resend_code = st.form_submit_button(
                    "🔄 Resend Code", use_container_width=True)

            if resend_code:
                if verification_email:
                    with st.spinner("Resending code..."):
                        verification_sent = session_manager.email_verification.send_verification_code(
                            verification_email)
                    ...
                    st.error(
                        "Error: No email address found to resend the code. Please go back and enter your email.")
                    st.session_state.verification_stage = "email_entry"
                st.rerun()
        ...
        # UNBLOCK CHAT (as it's non-blocking visual prompt)
        st.session_state.chat_blocked_by_dialog = False

        remaining_questions = session_manager.question_limits.question_limits[
            UserType.GUEST.value] - session.daily_question_count
        st.info(
            f"You chose not to verify the recognized email. You can still use your remaining **{remaining_questions} guest questions**.")
        st.info(
            "To ask more questions after this, or to save chat history, please verify your email.")
        ...
        with col_opts1:
            if st.button("📧 Enter a New Email for Verification", use_container_width=True, key="new_email_opt_btn"):
                ...
                st.session_state.verification_stage = "email_entry"
                # Reset if they change mind and go for new email
                st.session_state.guest_continue_active = False
                st.rerun()
        with col_opts2:
            if st.button("Continue as Guest for Now", use_container_width=True, key="continue_guest_btn"):
                ...
                st.session_state.verification_stage = None
                st.success(
                    "You can now continue as a Guest. The email prompt will reappear when your guest questions run out.")
                st.rerun()
...
def render_chat_interface_simplified(session_manager: 'SessionManager', session: UserSession, activity_result: Optional[Dict[str, Any]]):  # Merged from A and B
    ...
    # Initialize fingerprinting status if needed (from B)
    if 'fingerprint_status' not in st.session_state:
        ...
        # CRITICAL: Enable chat if no fingerprinting needed
        st.session_state.is_chat_ready = not fingerprint_needed

    # CRITICAL FIX: Don't start fingerprinting process if already done or not needed (from B)
    if fingerprint_needed and st.session_state.get('fingerprint_status') == 'pending':
        ...
        logger.info(
            f"Starting fingerprinting process for session {session.session_id[:8]} on chat page.")

    # Render fingerprinting component if needed (from B)
    if fingerprint_needed and st.session_state.get('fingerprint_status') == 'pending_js':
        ...
        if not st.session_state.get(fingerprint_key, False):
            try:
                session_manager.fingerprinting.render_fingerprint_component(
                    session.session_id)
                st.session_state[fingerprint_key] = True
                logger.info(
                    f"Triggered fingerprint component rendering for session {session.session_id[:8]} on chat page.")
            except Exception as e:
        ...
        wait_start = st.session_state.get(
            'fingerprint_wait_start', current_time_float)
        elapsed = current_time_float - wait_start
        # REDUCED: 10 seconds timeout instead of 15
        remaining = max(0, 10 - elapsed)

        if remaining > 0:
            st.info(
                f"🔒 **Securing your session...** ({remaining:.0f}s remaining)")
            st.caption(
                "FiFi is setting up device recognition for security and session management.")
            ...
            logger.warning(
                f"Fingerprint timeout (10s) for session {session.session_id[:8]} - enabling chat input with fallback.")
            st.rerun()

        return  # CRITICAL: Don't render rest of chat interface during fingerprinting

    # CRITICAL: If we reach here, fingerprinting is done or not needed (from B)
    ...
    if activity_result:
        ...
        if js_last_activity_timestamp:
            try:
                new_activity = datetime.fromtimestamp(
                    js_last_activity_timestamp / 1000)
                ...
                logger.error(
                    f"Failed to update activity from JavaScript: {e}")

    # Browser close detection for emergency saves (from A)
    ...
    # Display email prompt if needed AND get status to disable chat input (from A)
    should_disable_chat_input_by_dialog = display_email_prompt_if_needed(
        session_manager, session)

    # Render chat content ONLY if not blocked by a dialog (from A)
    if not st.session_state.get('chat_blocked_by_dialog', False):
        # ENHANCED: Show tier warnings for registered users
        limit_check_for_display = session_manager.question_limits.is_within_limits(
            session)
        if (session.user_type.value == UserType.REGISTERED_USER.value and
            ...
            if tier == 2 and remaining <= 3:
                st.warning(
                    f"⚠️ **Tier 2 Alert**: Only {remaining} questions remaining until 24-hour reset!")
            elif tier == 1 and remaining <= 2:
                st.info(
                    f"ℹ️ **Tier 1**: {remaining} questions remaining until 1-hour break.")
        ...
        for msg in visible_messages:
            with st.chat_message(msg.get("role", "user")):
                ...
                if msg.get("used_pinecone"):
                    indicators.append("🧠 FiFi Knowledge Base")
                if msg.get("used_search"):
                    indicators.append("🌐 FiFi Web Search")
                if msg.get("is_meta_response"):
                    indicators.append("📈 Session Analytics")  # Added from B
                if indicators:
                    st.caption(f"Enhanced with: {', '.join(indicators)}")

                if msg.get("safety_override"):
                    st.warning(
                        "🛡️ Safety Override: Switched to verified sources")
    ...
    # Rate limit notification with manual dismiss (from A)
    if 'rate_limit_hit' in st.session_state:
        ...
        elapsed = (current_time -
                   rate_limit_info['timestamp']).total_seconds()
        ...
        with col1:
            if remaining_time > 0:
                st.error(
                    f"⏱️ **Rate limit exceeded** - Please wait {remaining_time} seconds before asking another question. ({max_requests} questions per {window_seconds} seconds allowed)")
            else:
                st.error(
                    f"⏱️ **Rate limit exceeded** - Please wait a moment before asking another question.")
        with col2:
    ...
    # Content moderation notification with manual dismiss (from A)
    if 'moderation_flagged' in st.session_state:
        ...
        categories_text = ', '.join(
            categories) if categories else 'policy violation'
        message = moderation_info.get(
            'message', 'Your message violates our content policy.')

        col1, col2 = st.columns([5, 1])
        with col1:
            st.error(
                f"🛡️ **Content Policy Violation** - {categories_text}")
    ...
    # Context error notification with manual dismiss (from A)
    if 'context_flagged' in st.session_state:
        ...
        with col1:
            if category == "unrelated_industry":
                st.warning(
                    f"🏭 **Outside Food Industry** - This question doesn't relate to food & beverage ingredients.")
            elif category in ["personal_cooking", "off_topic"]:
                st.warning(
                    f"👨‍🍳 **Personal vs Professional** - I'm designed for B2B food industry questions.")
            else:
                st.warning(
                    f"🎯 **Off-Topic Question** - Please ask about food ingredients, suppliers, or formulation.")
    ...
    # NEW: Pricing/Stock notice with manual dismiss (from B)
            ...
            with st.spinner("🔍 FiFi is processing your question and we request your patience..."):
                try:
                    ...
                    if response.get('requires_email'):
                        ...
                        st.markdown(response.get(
                            "content", "No response generated."), unsafe_allow_html=True)
                        ...
                        if response.get("source"):
                            ...
                            st.caption(
                                f"{source_color} Source: {response['source']}")
...
def ensure_initialization_fixed():
    ...
    if 'initialized' not in st.session_state or not st.session_state.initialized:
        logger.info(
            "Starting application initialization sequence (no spinner shown, overlay is active)...")
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    f"Database manager initialization failed: {db_e}", exc_info=True)
            ...
            try:
                email_verification_manager = st.session_state.db_manager.EmailVerificationManager(
                    config)
            ...
            logger.critical(
                f"Critical initialization failure: {e}", exc_info=True)
            st.session_state.initialized = False
            return False

    return True

# Modified main function with proper loading state handling (from A)
...
def main_fixed():
    ...
    if loading_state:
        ...
        try:
            ...
            if not session_manager:
                set_loading_state(False)
                st.error(
                    "❌ Session Manager not available. Please refresh the page.")
                return
            ...
            if loading_reason == 'start_guest':
                ...
                if username and password:
                    authenticated_session = session_manager.authenticate_with_wordpress(
                        username, password)
                    if authenticated_session:
                        ...
                        if 'loading_reason' in st.session_state:
                            del st.session_state['loading_reason']
                        st.success(
                            f"🎉 Welcome back, {authenticated_session.full_name}!")
                        st.balloons()
                    else:
                        set_loading_state(False)
            ...
            if session:
                ...
                fingerprint_is_stable = not session.fingerprint_id.startswith(
                    ("temp_py_", "temp_fp_", "fallback_"))
                inheritance_checked = st.session_state.get(
                    f'fingerprint_checked_for_inheritance_{session.session_id}', False)

                if fingerprint_is_stable or inheritance_checked:
                    st.session_state.is_chat_ready = True
                    logger.info(
                        f"Chat input unlocked for session {session.session_id[:8]} after initial session/fingerprint setup.")
                else:
                    ...
                    st.session_state.is_chat_ready = False
                    logger.info(
                        f"Chat input remains locked for session {session.session_id[:8]} pending JS fingerprinting.")
            else:
    ...
    try:
        ...
        # Handle emergency saves and fingerprint data (from A)
        ...
        if current_page != "chat":
            ...
            if session is None or not session.active:
                logger.warning(
                    f"Expected active session for 'chat' page but got None or inactive. Forcing welcome page.")
                for key in list(st.session_state.keys()):
                    del st.session_state[key]
                st.session_state['page'] = None
                st.rerun()
            ...
            # 🔥 RENDER FINGERPRINTING FIRST, BEFORE TIMEOUT LOGIC (from A)
            fingerprint_needed = (
                session is not None and (
                    not session.fingerprint_id or
                    session.fingerprint_method == "temporary_fallback_python" or
                    session.fingerprint_id.startswith(
                        ("temp_py_", "temp_fp_", "fallback_"))
                )
            )
            ...
            # 🔥 NOW DO TIMEOUT LOGIC AFTER JAVASCRIPT IS RENDERED (from A)
            if session:
                fingerprint_is_stable = not session.fingerprint_id.startswith(
                    ("temp_py_", "temp_fp_", "fallback_"))
                ...
                if fingerprint_is_stable:
                    ...
                    # Use float for direct comparison with time.time()
                    current_time_float = time.time()
                    wait_start = st.session_state.get(
                        'fingerprint_wait_start')
                    ...
                    if wait_start is None:
                        ...
                        st.session_state.is_chat_ready = False
                        logger.info(
                            f"Starting fingerprint wait timer for session {session.session_id[:8]}")
                    elif current_time_float - wait_start > 15:  # ✅ 15 seconds is reasonable
                        # Timeout reached, enable chat with fallback fingerprint
                        st.session_state.is_chat_ready = True
                        logger.warning(
                            f"Fingerprint timeout (15s) - enabling chat with fallback for session {session.session_id[:8]}")
                    else:
            ...
            # Right after your timeout logic (from A)
            ...
            # Render activity tracker and check for timeout (from A)
            ...
            if session and session.session_id:
                ...
                if activity_tracker_key_state_flag not in st.session_state or \
                   st.session_state.get(f'{activity_tracker_key_state_flag}_session_id_check') != session.session_id:
                    ...
                    logger.info(
                        f"Rendering activity tracker component for session {session.session_id[:8]} at top level.")
                    activity_data_from_js = render_simple_activity_tracker(
                        session.session_id)
            ...
            timeout_triggered = check_timeout_and_trigger_reload(
                session_manager, session, activity_data_from_js)
            if timeout_triggered:
                return

            render_sidebar(session_manager, session,
                           st.session_state.pdf_exporter)
            render_chat_interface_simplified(
                session_manager, session, activity_data_from_js)
...
if __name__ == "__main__":
    try:  # Keep this from Codebase B for robust error handling
        main_fixed()
    except Exception as e:
